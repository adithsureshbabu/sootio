...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  try {
    ...
    // OPTIMIZATION: Fetch personal files and scrapers IN PARALLEL for faster results
    // Phase 0 + Phase 1 now run concurrently instead of sequentially
    ...
    // OPTIMIZATION: Await both promises in parallel - saves 200-500ms by not waiting for personal files first
    const [enrichedPersonalFiles, scraperResults] = await Promise.all([
      personalFilesPromise,
      scraperResultsPromise
    ]);

    console.log(`[${LOG_PREFIX}] Parallel fetch complete: ${enrichedPersonalFiles.length} personal files, scrapers returned`);

    let combinedResults = combineAndMarkResults(apiKey, enrichedPersonalFiles, scraperResults, specificSearchKey);
    ...
    const FOREGROUND_HASH_LIMIT = parseInt(process.env.FOREGROUND_CACHE_LIMIT, 10) || 50;
    ...
    const [sqliteCachedHashes, stremthruCachedHashes] = await Promise.all([
      // SQLite check (instant, local database)
      (async () => {
      ...
      // StremThru check (runs in parallel with SQLite)
      (async () => {
        const result = new Set();
    ...
    console.log(`[${LOG_PREFIX}] Parallel cache check completed in ${cacheCheckDuration}ms: SQLite=${sqliteCachedHashes.size}, StremThru=${stremthruCachedHashes.size}`)
