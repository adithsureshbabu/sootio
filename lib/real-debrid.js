...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  try {
    ...
    // Check only top 30 priority hashes in foreground for MAXIMUM speed (reduced from 100)
    const FOREGROUND_HASH_LIMIT = parseInt(process.env.FOREGROUND_CACHE_LIMIT, 10) || 30;
    const foregroundTorrents = prioritizedTorrents.slice(0, FOREGROUND_HASH_LIMIT);
    const foregroundHashes = [...new Set(foregroundTorrents.map(t => (t.InfoHash || t.hash || '').toLowerCase()).filter(Boolean))];

    console.log(`[${LOG_PREFIX}] Fast-path: checking ${foregroundHashes.length}/${allExternalHashes.length} priority hashes in foreground`);

    // OPTIMIZATION: Check SQLite FIRST (fast), then StremThru ONLY if needed
    // Check SQLite first (instant, local database)
    let sqliteCachedHashes = new Set();
    if (sqliteCache?.isEnabled()) {
      try {
        const results = await sqliteCache.getCachedHashes('realdebrid', foregroundHashes);
        for (const hash of results) {
          sqliteCachedHashes.add(hash.toLowerCase());
        }
        console.log(`[${LOG_PREFIX}] SQLite returned ${sqliteCachedHashes.size}/${foregroundHashes.length} cached hashes`);
      } catch (error) {
        console.error(`[${LOG_PREFIX}] Error getting SQLite cached hashes:`, error.message);
      }
    }

    // OPTIMIZATION: Only call StremThru if SQLite doesn't have ALL hashes
    // This saves 500ms+ when SQLite has everything!
    let stremthruCachedHashes = new Set();
    const needsStremThru = sqliteCachedHashes.size < foregroundHashes.length;
    ...
    if (needsStremThru && stremThru.isEnabled() && apiKey) {
      try {
        ...
        console.log(`[${LOG_PREFIX}] SQLite missing ${foregroundHashes.length - sqliteCachedHashes.size} hashes, checking StremThru...`);
        const start = Date.now();
        stremthruCachedHashes = await stremThru.checkInstantAvailability(foregroundHashes, 'realdebrid', apiKey, clientIp, sid);
        const duration = Date.now() - start;
        console.log(`[${LOG_PREFIX}] StremThru returned ${stremthruCachedHashes.size} cached hashes in ${duration}ms`);
      } catch (error) {
        console.error(`[${LOG_PREFIX}] Error querying StremThru:`, error.message);
      }
    } else if (!needsStremThru) {
      console.log(`[${LOG_PREFIX}] ⚡ SKIPPED StremThru (SQLite has all ${sqliteCachedHashes.size} hashes cached!)`);
    }
      ...
      const rdHandler = {
          ...
          checkCachedHashes: async (hashes) => {
              ...
              for (const hash of hashes) {
                ...
                if (foregroundCacheResults.has(hashLower)) {
                  cached.add(hashLower);
                } else {
                  uncheckedHashes.push(hash);
                }
              }
