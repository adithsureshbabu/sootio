...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  try {
    ...
    // Enrich torrents with stored episodeFileHint from SQLite (keyed by hash+releaseKey)
    if (episodeInfo && sqliteCache && sqliteCache.isEnabled()) {
      try {
        ...
        if (db) {
          ...
          for (const t of externalTorrents) {
            const hash = (normalizeInfoHash(t.InfoHash || t.hash || t.infoHash) || '').toLowerCase();
            if (!hash || t.episodeFileHint) continue;
            if (sqliteCachedHashes.has(hash)) {
              try {
                const row = stmt.get('realdebrid', hash, releaseKey);
                if (row && row.data) {
                  const parsed = typeof row.data === 'string' ? JSON.parse(row.data) : row.data;
                  if (parsed && parsed.episodeFileHint) {
                    t.episodeFileHint = parsed.episodeFileHint;
                    console.log(`[${LOG_PREFIX}] Enriched ${hash} with episodeFileHint from SQLite (${releaseKey})`);
                  }
                }
              } catch (e) {}
            }
