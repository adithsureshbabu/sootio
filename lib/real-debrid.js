...
async function findExistingTorrentIdByHash(RD, apiKey, hash, blockedStatuses = new Set(['magnet_error', 'error', 'virus', 'dead'])) {
  if (!hash) return null;

  for (let page = 1; page <= RD_EXISTING_TORRENT_SCAN_PAGES; page++) {
    const response = await rdCall(() => RD.torrents.get(0, page, RD_TORRENTS_PAGE_SIZE), apiKey);
    const torrentsList = Array.isArray(response?.data) ? response.data : [];
    const existing = torrentsList.find(t => {
      const torrentHash = String(t?.hash || '').toLowerCase();
      const status = String(t?.status || '').toLowerCase();
      return torrentHash === hash && !blockedStatuses.has(status);
    });
...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  try {
    ...
    // Enrich torrents with stored episodeFileHint from cache backend (keyed by hash+releaseKey)
    if (episodeInfo && sqliteCache && sqliteCache.isEnabled()) {
      try {
        const db = await sqliteCache.getDatabase();
        if (db) {
          const parseEpisodeHint = (row) => {
            if (!row || !row.data) return null;
            try {
              const parsed = typeof row.data === 'string' ? JSON.parse(row.data) : row.data;
              return parsed?.episodeFileHint || null;
            } catch {
              return null;
            }
          };
          ...
          const enrichable = [];
          for (const torrent of externalTorrents) {
            const hash = (normalizeInfoHash(torrent.InfoHash || torrent.hash || torrent.infoHash) || '').toLowerCase();
            if (!hash || torrent.episodeFileHint || !sqliteCachedHashes.has(hash)) continue;
            enrichable.push({ torrent, hash });
          }

          if (enrichable.length > 0) {
            if (typeof db.prepare === 'function') {
              const stmt = db.prepare(
                'SELECT data FROM cache WHERE service = ? AND hash = ? AND releaseKey = ? AND (expiresAt IS NULL OR expiresAt > CURRENT_TIMESTAMP) LIMIT 1'
              );

              for (const { torrent, hash } of enrichable) {
                const hint = parseEpisodeHint(stmt.get('realdebrid', hash, releaseKey));
                if (hint) {
                  torrent.episodeFileHint = hint;
                  console.log(`[${LOG_PREFIX}] Enriched ${hash} with episodeFileHint from cache (${releaseKey})`);
                }
              }
            } else if (typeof db.query === 'function') {
              const hashes = [...new Set(enrichable.map(item => item.hash))];
              const rows = await db.query(
                ...
                ['realdebrid', hashes, releaseKey]
              );
              const hintsByHash = new Map();
              for (const row of rows?.rows || []) {
                const hint = parseEpisodeHint(row);
                if (hint && row.hash) {
                  hintsByHash.set(String(row.hash).toLowerCase(), hint);
                }
              }

              for (const { torrent, hash } of enrichable) {
                const hint = hintsByHash.get(hash);
                if (hint) {
                  torrent.episodeFileHint = hint;
                  console.log(`[${LOG_PREFIX}] Enriched ${hash} with episodeFileHint from cache (${releaseKey})`);
                }
              ...
              console.warn(`[${LOG_PREFIX}] episodeFileHint enrichment skipped: unsupported cache database client`);
            }
          }
        }
      } catch (e) { console.error(`[${LOG_PREFIX}] episodeFileHint enrichment error:`, e.message); }
    }

    const cachedExternalTorrents = externalTorrents.filter(t => {
      if (!combinedCachedHashes.has((normalizeInfoHash(t.InfoHash || t.hash || t.infoHash) || '').toLowerCase())) return false;
      if (episodeInfo && !t.episodeFileHint) {
        const parsed = t.info || PTT.parse(t.Title || t.name || '');
        const isPack = (parsed.seasons && parsed.seasons.length > 0 || parsed.season != null) && !parsed.episode;
        if (isPack) return false;
      }
      return true;
    });
    const uncachedExternalTorrents = externalTorrents.filter(t => !cachedExternalTorrents.includes(t));
    ...
    // If we have 0 streams but uncached packs to inspect, run inspection synchronously
    if (allResults.length === 0 && uncachedExternalTorrents.length > 0 && episodeInfo) {
      console.log(`[${LOG_PREFIX}] 0 immediate streams but ${uncachedExternalTorrents.length} packs need inspection - running synchronously`);
      try {
        const RDsync = createRealDebridClient(apiKey, { clientIp });
        const syncHandler = {
          getIdentifier: () => LOG_PREFIX,
          isAborted: () => false,
          checkCachedHashes: async (hashes) => {
            if (sqliteCache?.isEnabled()) return await sqliteCache.getCachedHashes('realdebrid', hashes);
            return new Set();
          },
          getPersonalTorrentInfo: async (torrentId) => {
            try { return await rdCall(() => RDsync.torrents.info(torrentId), apiKey); } catch { return null; }
          },
          addMagnet: async (magnet) => {
            try { return await rdCall(() => RDsync.torrents.addMagnet(magnet), apiKey); } catch { return null; }
          },
          selectFiles: async (torrentId, fileIds) => {
            try { await rdCall(() => RDsync.torrents.selectFiles(torrentId, fileIds.join(',')), apiKey); } catch {}
          },
        ...
        const syncResults = await processAndFilterTorrents(uncachedExternalTorrents, syncHandler, episodeInfo, { byCategory: {}, byCategoryResolution: {} }, false);
        if (syncResults.length > 0) {
          if (sqliteCache?.isEnabled()) {
            const upserts = syncResults.map(t => ({
              service: 'realdebrid',
              ...
              category: t.category || getQualityCategory(t.name || t.Title || ''),
              resolution: torrentUtils.getResolutionFromName(t.name || t.Title || ''),
              data: { ...t, source: 'sync-pack-inspection', episodeFileHint: t.episodeFileHint || null, isCached: true }
            }));
            deferSqliteUpserts(uniqueUpserts(upserts));
          }
          allResults = [...allResults, ...syncResults.map(t => formatCachedResult(t, true))];
          console.log(`[${LOG_PREFIX}] Sync pack inspection found ${syncResults.length} streams`);
        }
      } catch (syncErr) {
        console.error(`[${LOG_PREFIX}] Sync pack inspection failed:`, syncErr.message);
      }
    }
...
async function processMagnetAlternative(apiKey, magnetUrl, clientIp) {
  ...
  try {
    ...
    await withRealDebridMagnetLock(apiKey, hash, async () => {
      torrentId = await findExistingTorrentIdByHash(RD, apiKey, hash, blockedStatuses);
      if (torrentId) {
        console.log(`[${LOG_PREFIX}] processMagnetAlternative: Reusing existing torrent ${torrentId} for hash ${hash.substring(0, 12)}...`);
        return;
      }

      const addResponse = await rdCall(() => RD.torrents.addMagnet(magnetUrl), apiKey);
      if (!addResponse?.data?.id) return;
      torrentId = addResponse.data.id;
      createdTorrent = true;
      console.log(`[${LOG_PREFIX}] processMagnetAlternative: Added new torrent ${torrentId}`);
    });
    if (!torrentId) return null;

    let info = await rdCall(() => RD.torrents.info(torrentId), apiKey);
    const hasSelectedFiles = Array.isArray(info?.data?.files)
      ? info.data.files.some(file => file?.selected)
      : false;
    if (!hasSelectedFiles || info?.data?.status === 'waiting_files_selection') {
      await rdCall(() => RD.torrents.selectFiles(torrentId, 'all'), apiKey);
    ...
    if (createdTorrent && torrentId) {
      try { await rdCall(() => RD.torrents.delete(torrentId), apiKey); } catch {}
    }
...
async function resolveMagnetUrl(apiKey, magnetUrl, clientIp) {
  ...
  try {
    ...
    if (hintPayload?.torrentId) {
      ...
      try {
        const info = await rdCall(() => RD.torrents.info(torrentId), apiKey);
        const status = String(info?.data?.status || '').toLowerCase();
        if (info?.data && !blockedStatuses.has(status)) {
          torrentData = info.data;
        } else {
          console.warn(`[${LOG_PREFIX} RESOLVER] Torrent ${torrentId} from hint is unusable (status: ${info?.data?.status}). Will search for another.`);
          torrentId = null;
        }
      } catch (e) {
