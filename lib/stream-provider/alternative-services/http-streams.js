 ...
 * Wraps 4KHDHub, HDHub4u, MKVCinemas, MKVDrama, MalluMv, CineDoze, UHDMovies, and MoviesDrive
...
export async function getHttpStreamingStreams(config, type, id, options = {}) {
  ...
  if (useMkvDrama) {
    addTask(
      'MkvDrama',
      buildCacheKey(id, 'mkvdrama-v2', season, episode),
  ...
  if (useNetflixMirror) {
    addTask(
      'NetflixMirror',
      buildCacheKey(id, 'netflixmirror', season, episode),
