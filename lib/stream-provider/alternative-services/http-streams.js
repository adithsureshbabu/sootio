 ...
 * Wraps 4KHDHub, HDHub4u, MKVCinemas, MalluMv, CineDoze, UHDMovies, MoviesDrive,
 * MoviesMod, MoviesLeech, AnimeFlix, 111477, and XDMovies
 */
...
export async function getHttpStreamingStreams(config, type, id, options = {}) {
  ...
  const use111477 = config.http111477 !== false;
  const useXDMovies = config.httpXDMovies === true;
  ...
  if (use111477) {
    addTask('111477', () => get111477Streams(id, type, season, episode, config, cinemetaDetails));
  }
