  ...
  try {
    ...
    if (provider === 'httpstreaming') {
      if (freshResults && freshResults.length > 0) {
        console.log(`[CACHE] Background refresh updating HTTP streaming cache for ${cacheKey} (${freshResults.length} items)`);
        await storeCacheResults(null, cacheKey, freshResults, type, provider);
      } else {
        console.log(`[CACHE] Background refresh: no HTTP streaming results to cache for ${cacheKey}`);
      }
