...
export async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (combinedResults.length < minResultsRequired) {
    ...
    if (freshResults.length > 0) {
      ...
      console.log(`[CACHE] Live check returned 0 results - NOT caching empty result to ensure future checks remain live`);
      // DO NOT cache empty results - this ensures we always check live when there are no results
      // This prevents stale zero-result caching and ensures new content is discovered quickly
    }

    return merged;
  }

  console.log(`[CACHE] Returning ${combinedResults.length} cached results immediately for ${cacheKey}`);
