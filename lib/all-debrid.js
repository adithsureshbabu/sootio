...
async function searchAllDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  const { signal, cleanup: cleanupAbort } = createAbortControllerWithTimeout(SEARCH_TIMEOUT_MS, 'AD');
  ...
  try {
    ...
    // Phase 1: Execute coordinated scrapers AND fetch cached results IN PARALLEL
    ...
    // Check cached scraper results in parallel - results from previous background scraper runs
    ...
    // Combine live scraper results with cached scraper results
    const allExternalResults = [...scraperResults, cachedScraperResults];
    let combinedResults = combineAndMarkResults(apiKey, personalFiles, allExternalResults, specificSearchKey);
    ...
    // Phase 2: Check for cached results - OPTIMIZED to avoid redundant StremThru calls
    ...
    // Phase 2a: Check cache store FIRST (instant, local database lookup)
    ...
    if (sqliteCache?.isEnabled()) {
        try {
          ...
          console.log(`[${LOG_PREFIX}] Cache returned ${sqliteCachedHashes.size}/${allExternalHashes.length} cached hashes`);
        } catch (error) {
          console.error(`[${LOG_PREFIX}] Error getting cache-store hashes:`, error.message);
        }
    }

    // Phase 2b: ONLY call StremThru if SQLite doesn't have ALL hashes (saves 500ms+ on cached requests)
    let stremthruCachedHashes = new Set();
    const needsStremThru = sqliteCachedHashes.size < allExternalHashes.length;

    if (needsStremThru && stremThru.isEnabled() && apiKey) {
      try {
        ...
        console.log(`[${LOG_PREFIX}] Cache missing ${allExternalHashes.length - sqliteCachedHashes.size} hashes, checking StremThru...`);
    ...
    } else if (!needsStremThru) {
      console.log(`[${LOG_PREFIX}] ⚡ SKIPPED StremThru (cache has all ${sqliteCachedHashes.size} hashes cached!)`);
    }
    
    // Phase 4: Combine all cached results (StremThru = others' cached, cache store = user's personal, Torz = confirmed cached) and return immediately
    ...
    console.log(`[${LOG_PREFIX}] Total cached hashes: ${combinedCachedHashes.size} (StremThru: ${stremthruCachedHashes.size}, Cache: ${sqliteCachedHashes.size}, Torz confirmed: ${confirmedCachedHashes.size})`);
    ...
    // Save initially cached results to cache store with full metadata (from StremThru/cache check)
      ...
      if (sqliteCache?.isEnabled()) {
          try {
              ...
              if (sqliteHashes.length > 0) {
                  console.log(`[AD CACHE-CHECK] Cache returned ${sqliteHashes.length}/${hashes.length} cached hashes`);
              }
          } catch (error) {
              console.error(`[AD CACHE] Error getting cached hashes: ${error.message}`);
          }
      }
