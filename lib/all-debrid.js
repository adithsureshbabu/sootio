...
async function searchAllDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  try {
    ...
    // Phase 2a: Check SQLite cache FIRST (instant, local database lookup)
    ...
    if (sqliteCache?.isEnabled()) {
        try {
          ...
          console.log(`[${LOG_PREFIX}] SQLite returned ${sqliteCachedHashes.size}/${allExternalHashes.length} cached hashes`);
        } catch (error) {
    ...
    if (needsStremThru && stremThru.isEnabled() && apiKey) {
      try {
        ...
        console.log(`[${LOG_PREFIX}] SQLite missing ${allExternalHashes.length - sqliteCachedHashes.size} hashes, checking StremThru...`);
