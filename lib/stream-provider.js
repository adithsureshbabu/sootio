...
const EARLY_RETURN_ENABLED = process.env.DISABLE_EARLY_RETURN !== 'true';
...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (hasSufficientCache) {
    ...
    // Return cached results immediately WITHOUT waiting for scrapers
    // Note: Personal files are fetched in background refresh
    return combinedResults;
  }
