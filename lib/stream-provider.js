import { get4KHDHubStreams, getHDHub4uStreams, getMKVCinemasStreams, getVixSrcStreams } from './http-streams.js';
...
const HTTP_STREAMING_TIMEOUT_MS = parseInt(process.env.HTTP_STREAMING_TIMEOUT_MS) || 20000; // 20 seconds default for HTTP streams
const HTTP_STREAMING_EARLY_RETURN_MS = parseInt(process.env.HTTP_STREAMING_EARLY_RETURN_MS) || 4000; // Target <4s for initial HTTP streams
const HTTP_STREAMING_MIN_RESULTS = parseInt(process.env.HTTP_STREAMING_MIN_RESULTS) || 1; // Minimum streams needed to return early
...
/**
 * Fast-path aggregator for HTTP streaming providers.
 * Returns as soon as we have enough streams or the short budget expires,
 * while allowing slower providers to keep running for cache warming.
 */
function runHttpStreamingWithEarlyReturn(promises, contextLabel = 'httpstreaming') {
  ...
  return new Promise(resolve => {
    ...
    const finalize = (reason) => {
      ...
      console.log(`[HTTP-STREAMS] ${contextLabel} returning ${flat.length} stream(s) (${reason})`);
      resolve(flat);
    };
    ...
    promises.forEach(promise => {
      promise.then(streams => {
        ...
        if (totalCount >= HTTP_STREAMING_MIN_RESULTS) {
          finalize('early threshold reached');
        } else if (settled === promises.length) {
          finalize('all providers finished');
        }
      }).catch(err => {
        console.warn(`[HTTP-STREAMS] Provider failed: ${err?.message || err}`);
        settled++;
        if (settled === promises.length) {
          finalize('all providers finished (errors allowed)');
        }
      });
    });
    ...
    setTimeout(() => {
      if (!resolved && totalCount > 0) {
        finalize('timeout with partial results');
      }
    }, HTTP_STREAMING_EARLY_RETURN_MS);
  });
}
...
async function getCachedTorrents(provider, type, id, config, searchFn, personalFetchFn = null) {
  ...
  // OPTIMIZATION: Check if cache has sufficient tier fulfillment
  // If yes, return cached results + personal files immediately and refresh in background
  const hasSufficientCache = cached && combinedResults.length > 0 && hasSufficientTierFulfillment(combinedResults);

  if (hasSufficientCache) {
    console.log(`[CACHE] ⚡ TIER FULFILLMENT MET - returning ${combinedResults.length} cached results IMMEDIATELY (skipping scrapers!) for ${cacheKey}`);
    ...
    // Return cached results immediately, but merge in personal files (not cached) for this user
    if (typeof personalFetchFn === 'function') {
      try {
        const personalResults = await personalFetchFn();
        if (personalResults?.length) {
          console.log(`[CACHE] Merged ${personalResults.length} personal results with cached data for ${cacheKey}`);
          return mergePersonalResults(personalResults, combinedResults);
        }
      } catch (err) {
  ...
  // For debrid services when cache is insufficient OR empty:
  ...
  if (cached && combinedResults.length > 0) {
    console.log(`[CACHE] ⚠️ TIER FULFILLMENT NOT MET - running scrapers in foreground for ${cacheKey}`);
  } else {
...
async function getMovieStreams(config, type, id) {
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        const httpStreamTasks = [];

        if (use4KHDHub) {
          httpStreamTasks.push(
            withTimeout(
        ...
        if (useVixSrc) {
          httpStreamTasks.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${vixSrcId}-vixsrc`, config, () =>
        ...
        if (httpStreamTasks.length > 0) {
          allStreamsPromises.push(runHttpStreamingWithEarlyReturn(httpStreamTasks, 'movie-httpstreaming'));
        }
  ...
  if (EARLY_RETURN_ENABLED && allStreamsPromises.length > 1) {
    ...
    if (raceResult === 'timeout') {
      ...
      console.log(`[PARALLEL-SEARCH] ⏱️ Early return check: ${completedResults.length} providers complete, ${currentStreams.length} streams collected (need ${EARLY_RETURN_MIN_STREAMS})`);
    ...
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        if (useVixSrc) {
          httpStreamTasks.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${vixSrcId}-vixsrc-${season}:${episode}`, config, () =>
        ...
        if (httpStreamTasks.length > 0) {
          allStreamsPromises.push(runHttpStreamingWithEarlyReturn(httpStreamTasks, 'series-httpstreaming'));
        }
...
async function resolveUrl(debridProvider, debridApiKey, itemId, hostUrl, clientIp, config = {}) {
  ...
  const cacheKey = buildResolveCacheKey(provider, debridApiKey, hostUrl);
  if (useResolveCache) {
