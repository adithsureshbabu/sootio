import { get4KHDHubStreams, getHDHub4uStreams, getMKVCinemasStreams, getMalluMvStreams, getCineDozeStreams, getVixSrcStreams, getMkvDramaStreams, getNetflixMirrorStreams } from './http-streams.js';
...
import { filterByEpisode } from './stream-provider/utils/filtering.js';
...
function getHttpStreamingTimeout(serviceName) {
  ...
  if (providerKey === 'MKVDRAMA') {
    ...
    return Math.max(HTTP_STREAMING_TIMEOUT_MS, 45000);
  }
...
function getCacheResultKey(item, provider) {
  ...
  if ((providerKey === 'httpstreaming' || providerKey === 'easynews' || providerKey === 'debriderapp' || providerKey === 'personalcloud') && item.url) {
...
async function getCachedTorrents(provider, type, id, config, searchFn, personalFetchFn = null) {
  ...
  const shouldFetchTorz = searchResults.length < minResultsRequired;

  // Only check Torz in the foreground if cache doesn't meet the minimum result count
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      ...
      if (service.provider === 'Usenet') {
        ...
        const useMkvDrama = service.httpMkvDrama !== false;  // Default to true if not specified
        ...
        const useNetflixMirror = service.httpNetflixMirror !== false;
        ...
        if (use4KHDHub) {
          addStreamTask(
            ...
            { provider: '4KHDHub', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        if (useHDHub4u) {
          addStreamTask(
            ...
            { provider: 'HDHub4u', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        if (useMKVCinemas) {
          addStreamTask(
            ...
            { provider: 'MKVCinemas', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        if (useMkvDrama) {
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-mkvdrama-v2`, config, () =>
        ...
        if (useCineDoze) {
          addStreamTask(
            ...
            { provider: 'CineDoze', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        if (useMalluMv) {
          addStreamTask(
            ...
            { provider: 'MalluMv', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        if (useUHDMovies) {
          addStreamTask(
            ...
            { provider: 'UHDMovies', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        if (useMoviesDrive) {
          addStreamTask(
            ...
            { provider: 'MoviesDrive', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        if (useVixSrc) {
          addStreamTask(
            ...
            { provider: 'VixSrc', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        if (useNetflixMirror) {
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-netflixmirror`, config, () =>
                getNetflixMirrorStreams(id, type, null, null, config, cinemetaDetails)
              ).then(streams => wrapHttpStreamsWithResolver(streams, config.host)),
              getHttpStreamingTimeout('NetflixMirror'),
              'NetflixMirror'
            ),
            { provider: 'NetflixMirror', earlyReturnDisabled: true, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
    ...
    config.DebridServices.forEach(service => {
      ...
      if (service.provider === 'Usenet') {
        ...
        if (useMkvDrama) {
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${imdbId}-mkvdrama-v2-${season}:${episode}`, config, () =>
        ...
        if (useNetflixMirror) {
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${imdbId}-netflixmirror-${season}:${episode}`, config, () =>
