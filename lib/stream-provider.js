...
async function getCachedTorrents(provider, type, id, config, searchFn, personalFetchFn = null, options = null) {
  if (personalFetchFn && typeof personalFetchFn === 'object' && options == null) {
    options = personalFetchFn;
    personalFetchFn = null;
  }

  const onCacheStatus = options?.onCacheStatus;

  if (!SqliteCache.isEnabled()) {
    if (typeof onCacheStatus === 'function') {
      try {
        onCacheStatus({ provider, cacheKey: null, cacheHit: false, hasCachedResults: false });
      } catch (error) {
        console.error(`[CACHE] onCacheStatus callback failed for ${provider}: ${error.message}`);
      }
    }
    return searchFn();
  }
  ...
  const hasCachedResults = searchResults.length > 0;

  if (typeof onCacheStatus === 'function') {
    try {
      onCacheStatus({ provider, cacheKey, cacheHit: Boolean(cached), hasCachedResults });
    } catch (error) {
      console.error(`[CACHE] onCacheStatus callback failed for ${provider}: ${error.message}`);
    }
  }

  // Only check Torz in the foreground if cache has no results
  const torzResults = hasCachedResults ? [] : await fetchTorzResults(provider, type, id, config);
  ...
  // Warm personal files cache in the background when serving from cache
  if (typeof personalFetchFn === 'function') {
    setImmediate(() => {
      personalFetchFn()
        .then(personalResults => {
          if (personalResults?.length) {
            console.log(`[CACHE] Personal files refreshed in background for ${cacheKey}`);
          }
        })
        .catch(err => console.error(`[CACHE] Personal fetch failed for ${cacheKey}:`, err.message));
    });
  }
...
async function refreshCacheInBackground(provider, type, id, config, searchFn, cacheKey, existingResults) {
  try {
    ...
    const [freshResults, torzResults] = await Promise.all([
      searchFn(true),
      fetchTorzResults(provider, type, id, config)
    ]);
...
async function getMovieStreams(config, type, id) {
  ...
  const allStreamsMeta = [];
  const addStreamPromise = (promise, meta = {}) => {
    const index = allStreamsPromises.length;
    allStreamsPromises.push(promise);
    allStreamsMeta.push(meta);
    return index;
  };
  const addHttpStreamTask = (label, cacheKey, searchFn, transform = null) => {
    const meta = { blockEarlyReturn: true };
    const transformFn = transform || (streams => wrapHttpStreamsWithResolver(streams, config.host));
    const promise = withTimeout(
      getCachedTorrents('httpstreaming', type, cacheKey, config, searchFn, null, {
        onCacheStatus: status => {
          meta.blockEarlyReturn = !status?.hasCachedResults;
        }
      }).then(transformFn),
      HTTP_STREAMING_TIMEOUT_MS,
      label
    );
    addStreamPromise(promise, meta);
  };
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        const useMKVCinemas = service.httpMKVCinemas !== false;  // Default to true if not specified
        const useMalluMv = service.httpMalluMv !== false;  // Default to true if not specified
        const useCineDoze = service.httpCineDoze !== false;  // Default to true if not specified
        const useVixSrc = service.httpVixSrc !== false;
  ...
  if (EARLY_RETURN_ENABLED && allStreamsPromises.length > 1) {
    ...
    // Create a promise that resolves after the early return timeout
    const earlyReturnTimer = new Promise(resolve => {
      setTimeout(() => resolve('timeout'), EARLY_RETURN_TIMEOUT_MS);
    });

    // Wrap each promise to track completion
    const trackedPromises = allStreamsPromises.map((promise, index) =>
      promise.then(result => {
        completedResults.push({ index, result, status: 'fulfilled', meta: allStreamsMeta[index] });
        return result;
      }).catch(err => {
        completedResults.push({ index, result: [], status: 'rejected', error: err, meta: allStreamsMeta[index] });
        return [];
      })
    );

    // Race between all promises completing and the early return timer
    const raceResult = await Promise.race([
      Promise.all(trackedPromises).then(() => 'all-complete'),
      earlyReturnTimer
    ]);
    ...
    if (raceResult === 'timeout') {
      ...
      const blockingCount = allStreamsMeta.filter(meta => meta?.blockEarlyReturn).length;
      const blockingCompleted = completedResults.filter(r => r.meta?.blockEarlyReturn).length;
      const waitingOnBlocking = blockingCompleted < blockingCount;
      console.log(`[PARALLEL-SEARCH] ⏱️ Early return check: ${completedResults.length} providers complete, ${currentStreams.length} streams collected (need ${EARLY_RETURN_MIN_STREAMS})${waitingOnBlocking ? ' - waiting on HTTP streams' : ''}`);
      if (!waitingOnBlocking && currentStreams.length >= EARLY_RETURN_MIN_STREAMS) {
  ...
  const totalDuration = Date.now() - totalStartTime;
  console.log(`[PARALLEL-SEARCH] ✅ Returning ${flatStreams.length} streams after ${totalDuration}ms`);
    ...
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        if (useMKVCinemas) {
          addHttpStreamTask(
            'MKVCinemas',
            `${imdbId}-mkvcinemas-${season}:${episode}`,
        ...
        if (useCineDoze) {
          addHttpStreamTask(
            'CineDoze',
            `${imdbId}-cinedoze-${season}:${episode}`,
        ...
        if (useUHDMovies) {
          addHttpStreamTask(
            'UHDMovies',
            `${imdbId}-uhdmovies-${season}:${episode}`,
        ...
        if (useMoviesDrive) {
          addHttpStreamTask(
            'MoviesDrive',
            `${imdbId}-moviesdrive-${season}:${episode}`,
        ...
        if (useVixSrc) {
          addHttpStreamTask(
            'VixSrc',
            `${vixSrcId}-vixsrc-${season}:${episode}`,
...
function toStream(details, type, config, streamHint = {}) {
  ...
  const streamObj = {
    ...
    behaviorHints: {
      bingeGroup: `${normalizedSource}|${details.hash || details.id || 'unknown'}`
    }
  };
