...
async function getMovieStreams(config, type, id) {
  ...
  if (EARLY_RETURN_ENABLED && allStreamsPromises.length > 1) {
    ...
    if (raceResult === 'timeout') {
      ...
      if (pendingProviders === 0 && currentStreams.length >= EARLY_RETURN_MIN_STREAMS) {
      ...
      } else if (pendingProviders > 0) {
        console.log(`[PARALLEL-SEARCH] ⏳ Waiting for ${pendingProviders} remaining provider(s) to finish before returning`);
      }
    }
