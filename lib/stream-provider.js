...
import * as ResponseCache from './util/response-cache.js';
...
async function getMovieStreamsDeduped(config, type, id) {
  // Check response cache first (ultra-fast)
  const cached = ResponseCache.getCached(config, type, id);
  if (cached) {
    return cached;
  }
  ...
  const result = await dedupedRequest(key, () => getMovieStreams(config, type, id));

  // Cache the response for ultra-fast future requests
  ResponseCache.setCached(config, type, id, result);
...
async function getSeriesStreamsDeduped(config, type, id) {
  ...
  const result = await dedupedRequest(key, () => getSeriesStreams(config, type, id));
