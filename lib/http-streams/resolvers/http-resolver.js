...
const FAST_SEEK_TIMEOUT_MS = parseInt(process.env.HTTP_STREAM_SEEK_TIMEOUT_MS, 10) || 2500;
...
const PRIORITY_PATTERNS = [
    // Mega Server - HIGHEST PRIORITY (user prefers Mega fallback)
...
async function findSeekableLink(results, { timeoutMs = FAST_SEEK_TIMEOUT_MS, maxParallel = MAX_PARALLEL_VALIDATIONS } = {}) {
    ...
    // Build a candidate list with priority links first and deduplicate by URL
    const seen = new Set();
    const candidates = [];
    ...
    for (const { pattern, name } of PRIORITY_PATTERNS) {
        ...
        if (candidate && !seen.has(candidate.url)) {
            candidates.push({ candidate, label: name });
    ...
    for (const candidate of results) {
        ...
        candidates.push({ candidate, label: hostname });
        seen.add(candidate.url);
    }
