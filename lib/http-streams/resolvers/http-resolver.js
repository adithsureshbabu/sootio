...
function collectViewcrateEntries(document, baseUrl) {
    ...
    blockSelectors.forEach(({ selector, episodeAttr }) => {
        document(selector).each((_, block) => {
            ...
            $block.find('.y_u5qme, .y_tpl1j').each((__, entry) => {
                const $entry = document(entry);
                const hostAttr = $entry.attr('data-ogehf') || $entry.attr('data-7kuiu') || '';
                let host = hostAttr.toLowerCase();
                if (!host) {
                    const hostText = $entry.find('.w_po9rr, .w_4vj7h').first().text().trim();
                    host = hostText.toLowerCase();
                }

                const filename = $entry.find('.x_qwwj2, .x_i29qt').first().text().trim() ||
...
function orderViewcrateCandidates(candidates, hints = {}) {
    ...
    if (hostHint && hostHint !== 'gofile') {
        filtered = filtered.filter(candidate => !candidate?.host?.toLowerCase().includes('gofile'));
    }
...
async function resolveOuoLink(shortUrl, hints = {}) {
    ...
    for (let step = 0; step < maxSteps; step += 1) {
        ...
        if (directFromPage) {
            ...
            if (directHost === 'gofile' && hintHost && hintHost !== 'gofile') {
                console.log('[HTTP-RESOLVE] Skipping GoFile link due to host hint');
            } else {
