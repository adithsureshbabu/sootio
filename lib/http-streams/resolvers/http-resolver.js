...
const CF_COOKIE_CACHE_TTL = parseInt(process.env.CF_COOKIE_CACHE_TTL, 10) || (15 * 60 * 1000); // 15 minutes
...
function getCachedCfCookies(domain) {
    ...
    if (Date.now() - cached.ts > CF_COOKIE_CACHE_TTL) {
    ...
    try {
        ...
        if (response.data?.status === 'ok' && response.data?.solution?.response) {
            ...
            if (domain && solutionCookies) {
                cacheCfCookies(domain, solutionCookies, userAgent || OUO_USER_AGENT);
            }
...
export async function resolveHttpStreamUrl(redirectUrl) {
    ...
    const resolverPromise = (async () => {
        ...
        // Hard short-circuit for NetflixMirror HLS links even without provider param
        const lowerUrl = (workingUrl || '').toLowerCase();
        if (lowerUrl.includes('net51.cc') || lowerUrl.includes('net20.cc') || lowerUrl.includes('nfmirror')) {
            console.log('[HTTP-RESOLVE] NetflixMirror link detected early, returning without 206 validation');
        ...
        if (workingUrl.includes('hubcloud') || workingUrl.includes('hubdrive') || workingUrl.includes('4khdhub')) {
            ...
            provider = 'HDHub4u';
        } else if (workingUrl.includes('net51.cc') || workingUrl.includes('net20.cc') || workingUrl.includes('nfmirror')) {
            provider = 'NetflixMirror';
        }
        console.log('[HTTP-RESOLVE] Detected provider:', provider);

        // NetflixMirror HLS links should not be 206-validated; return directly
        if (provider === 'NetflixMirror') {
            console.log('[HTTP-RESOLVE] NetflixMirror link detected, returning without 206 validation');
            resolveCache.set(cacheKey, { value: workingUrl, ts: Date.now() });
            return workingUrl;
        }
