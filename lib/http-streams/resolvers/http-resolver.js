...
async function resolveXDMoviesRedirect(url) {
    if (!url || !url.includes(XDMOVIES_LINK_HOST)) {
        return url;
    }
...
async function getOrCreateFlareSession(domain) {
    ...
    const proxyConfig = FLARESOLVERR_PROXY_URL ? { url: FLARESOLVERR_PROXY_URL } : null;
...
async function fetchWithCloudflare(url, options = {}) {
    ...
    if (sawChallenge || sawError || preferFlareSolverr) {
        console.log('[HTTP-RESOLVE] Cloudflare challenge/error detected, using FlareSolverr');
...
export async function resolveHttpStreamUrl(redirectUrl) {
    ...
    const resolverPromise = (async () => {
        ...
        if (workingUrl.includes(XDMOVIES_LINK_HOST)) {
