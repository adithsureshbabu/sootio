...
export async function resolveHttpStreamUrl(redirectUrl) {
    ...
    const resolverPromise = (async () => {
        ...
        if (decodedUrl.includes('gdlink.dev')) {
            console.log('[HTTP-RESOLVE] gdlink.dev detected, attempting extractor resolution');
            try {
                const extracted = await processExtractorLinkWithAwait(decodedUrl, 99) || [];
        ...
        if (decodedUrl.includes('cinedoze.tv/links/')) {
            ...
            try {
                ...
                const response = await makeRequest(decodedUrl, { parseHTML: false, timeout: 12000 });
