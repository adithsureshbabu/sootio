...
async function resolveXDMoviesProtectorUrl(url) {
    ...
    try {
        ...
        const protectorBase = `${parsed.protocol}//${parsed.host}`;
        ...
        try {
            ...
            console.log('[HTTP-RESOLVE] XDMovies protector session response was not JSON');
            return url;
        }

        if (!sessionData?.sessionId || !sessionData?.token) {
            console.log('[HTTP-RESOLVE] XDMovies protector session missing sessionId/token');
            return url;
        }
        ...
        const finalLocation = goResponse.headers?.location || goResponse.headers?.Location;
        if (finalLocation) {
            return new URL(finalLocation, protectorBase).toString();
        }
    } catch (err) {
