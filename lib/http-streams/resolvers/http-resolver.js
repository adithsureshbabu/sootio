...
const GOFILE_WEBSITE_TOKEN_DEFAULT = '4fd6sg89d7s6';
...
async function resolveViewcrateGetLink(getUrl, referer, hints = {}) {
    ...
    if (GOFILE_HOSTS.some(host => directUrl?.toLowerCase().includes(host))) {
        ...
        return resolveGofileDownload(directUrl, hints);
    }
    ...
    if (!PIXELDRAIN_HOSTS.some(host => normalized.toLowerCase().includes(host))) {
        ...
        if (GOFILE_HOSTS.some(host => normalized.toLowerCase().includes(host))) {
            return resolveGofileDownload(normalized, hints);
        }
        return normalized;
    }
...
function extractGofileContentId(gofileUrl) {
    if (!gofileUrl) return null;
    const match = gofileUrl.match(/gofile\.io\/d\/([A-Za-z0-9]+)/i);
    return match?.[1] || null;
}

function normalizeGofilePassword(password) {
...
function getGofilePasswordHashFromUrl(gofileUrl) {
    try {
        const parsed = new URL(gofileUrl);
        const raw = parsed.searchParams.get('password') || parsed.searchParams.get('pw') || null;
        return normalizeGofilePassword(raw);
    } catch {
        return null;
    }
}
...
async function fetchGofileContent(contentId, { accountToken = null, websiteToken = null, passwordHash = null } = {}) {
    ...
    if (passwordHash) {
        contentUrl.searchParams.set('password', passwordHash);
    }
    ...
    const headers = {
        ...
        'Origin': 'https://gofile.io',
        'Referer': 'https://gofile.io/',
        'X-Website-Token': websiteToken || GOFILE_WEBSITE_TOKEN_DEFAULT
    };
    ...
    try {
        ...
        console.log(`[HTTP-RESOLVE] GoFile: API error: ${error.message}`);
        return null;
    }
}

function buildGofileCandidate(name, link, pathParts = []) {
    const safeName = name || '';
    const fullPath = [...pathParts, safeName].filter(Boolean).join('/');
    const label = fullPath || safeName || link || '';
    return {
        name: safeName,
        link,
        label,
        resolution: getResolutionFromName(label)
    };
}

function matchesGofileEpisode(label, episodeKey) {
    if (!episodeKey) return true;
    const compactLabel = String(label || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const compactEpisode = String(episodeKey).toLowerCase().replace(/[^a-z0-9]/g, '');
    if (compactEpisode && compactLabel.includes(compactEpisode)) {
        return true;
    }

    const match = String(episodeKey).match(/S(\d{1,2})E(\d{1,3})/i);
    if (match) {
        const episodeNumber = parseInt(match[2], 10);
        if (!Number.isNaN(episodeNumber)) {
            const episodeRegex = new RegExp(`\\b(?:ep(?:isode)?\\s*0*${episodeNumber}|e0*${episodeNumber})\\b`, 'i');
            return episodeRegex.test(label || '');
        }
    }
...
function orderGofileCandidates(candidates, hints = {}) {
    if (!candidates.length) return [];

    let filtered = candidates;

    if (hints.episode) {
        filtered = filtered.filter(candidate => matchesGofileEpisode(candidate.label, hints.episode));
    }
...
function pickGofileCandidate(candidates, hints = {}) {
    const ordered = orderGofileCandidates(candidates, hints);
    if (!ordered.length) return null;

    const videoCandidates = ordered.filter(candidate => {
        const evaluation = evaluateVideoCandidate({
            url: candidate.link,
            title: candidate.label,
            name: candidate.name
        });
        return evaluation.isVideo;
    });

    return videoCandidates[0] || ordered[0] || null;
}

async function collectGofileFiles(contentId, options = {}, state = null, pathParts = []) {
    if (!contentId) return [];
    const tracker = state || { visited: new Set() };
    if (tracker.visited.has(contentId)) return [];
    tracker.visited.add(contentId);

    const contentData = await fetchGofileContent(contentId, options);
    if (!contentData || contentData.status !== 'ok' || !contentData.data) {
    ...
    if (data.type && data.type !== 'folder') {
        if (!data.link) return [];
        return [buildGofileCandidate(data.name, data.link, pathParts)];
    }

    const folderName = data.name || '';
    const includeFolder = folderName && !(pathParts.length === 0 && folderName.toLowerCase() === 'root');
    const nextPathParts = includeFolder ? [...pathParts, folderName] : pathParts;
    ...
    for (const child of children) {
        ...
        if (child.type === 'folder' && child.id) {
            try {
                const nested = await collectGofileFiles(child.id, options, tracker, nextPathParts);
                if (nested.length) {
                    results.push(...nested);
                }
            } catch (error) {
                console.log(`[HTTP-RESOLVE] GoFile: Failed to traverse folder ${child.id}: ${error.message}`);
            }
        } else if (child.type === 'file' && child.link) {
            results.push(buildGofileCandidate(child.name, child.link, nextPathParts));
        }
    }
 ...
 */
async function resolveGofileDownload(gofileUrl, hints = {}) {
    if (!gofileUrl) return null;
    ...
    try {
        ...
        const passwordHash = getGofilePasswordHashFromUrl(gofileUrl);
        const files = await collectGofileFiles(contentId, {
        ...
        const selected = pickGofileCandidate(files, hints);
        if (!selected?.link) {
            console.log('[HTTP-RESOLVE] GoFile: No matching file found in content');
            return null;
        }

        console.log(`[HTTP-RESOLVE] GoFile: Found file "${selected.name || selected.label || 'unknown'}"`);
        if (guestToken) {
            cacheGofileToken(selected.link, { accountToken: guestToken, websiteToken });
        }
...
export async function resolveHttpStreamUrl(redirectUrl) {
    ...
    const resolverPromise = (async () => {
        ...
        if (GOFILE_HOSTS.some(host => workingUrl.includes(host)) && workingUrl.includes('/d/')) {
            ...
            try {
                const resolved = await resolveGofileDownload(workingUrl, hints);
                if (resolved) {
