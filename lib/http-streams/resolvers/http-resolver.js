...
const SHORTLINK_INTERSTITIAL_HOSTS = ['advertisingcamps.com', 'crn77.com', 'clk.sh'];
...
const VIEWCRATE_BROWSER_FALLBACK_ENABLED = process.env.MKVDRAMA_BROWSER_FALLBACK_ENABLED !== 'false';
...
function shouldUseFlareProxyForDomain(domain) {
    ...
    // Disable proxy for shortlink/container services - they need direct access to execute JS properly
    if (OUO_HOSTS.some(host => lower.includes(host)) ||
        VIEWCRATE_HOSTS.some(host => lower.includes(host)) ||
...
function shouldUseFlareSessionForDomain(domain) {
    ...
    const lower = domain.toLowerCase();
    return !OUO_HOSTS.some(host => lower.includes(host));
}
...
function isShortlinkInterstitialUrl(url) {
    ...
    try {
        ...
        return SHORTLINK_INTERSTITIAL_HOSTS.some(host => lower === host || lower.endsWith(`.${host}`));
    } catch {
        const lower = String(url).toLowerCase();
        return SHORTLINK_INTERSTITIAL_HOSTS.some(host => lower.includes(host));
    }
}
...
async function resolveOuoLink(shortUrl, hints = {}) {
    ...
    for (let step = 0; step < maxSteps; step += 1) {
        ...
        if (!form || !form.length) {
            ...
            const browserResolved = await resolveOuoLinkWithBrowser(request.url, hints);
            if (browserResolved) {
