...
const SQLITE_SERVICE_KEY = 'mkvcinemas';
...
async function getDbCached(hashKey, ttl) {
    ...
    try {
        ...
        console.error(`[MKVCinemas] Failed to read db cache: ${error.message}`);
    }
    return null;
}
...
async function setDbCache(hashKey, data, ttlMs) {
    ...
    try {
        await SqliteCache.upsertCachedMagnet({
            ...
            hash: hashKey,
            data,
            releaseKey: 'mkvcinemas-http-streams'
        }, { ttlMs });
    } catch (error) {
        console.error(`[MKVCinemas] Failed to write db cache: ${error.message}`);
    }
}
...
export async function scrapeMKVCinemasSearch(query, signal = null) {
    ...
    try {
        ...
        // Cache the results (memory + db) - only if we have results
        if (results.length > 0) {
...
export async function loadMKVCinemasContent(postUrl, signal = null) {
    ...
    // Check db cache (only use if we have actual download pages)
    const dbCached = await getDbCached(dbCacheKey, CONTENT_CACHE_TTL);
    if (dbCached?.downloadPages?.length > 0) {
        console.log(`[MKVCinemas] Content cache hit (db) for ${postUrl}`);
        contentCache.set(postUrl, { data: dbCached, ts: Date.now() });
        return dbCached;
    }
    ...
    try {
        ...
        // Cache the result (memory + db) - only if we have download pages
