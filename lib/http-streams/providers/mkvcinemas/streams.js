...
export async function getMKVCinemasStreams(tmdbId, type, season = null, episode = null, config = {}, signal = null) {
    try {
        let meta = await Cinemeta.getMeta(type, tmdbId);
        if (!meta?.name) {
        ...
        const resolvedOptions = await mapWithConcurrency(
            ...
            async ({ opt, sourcePage }) => {
                ...
                let label = (inferredName || `${opt.quality || content.title || meta.name}`).trim();
                // gdlink.dev URLs only expose an ID in the path; use content title + quality for a meaningful label
                if (resolvedUrl.includes('gdlink.dev')) {
