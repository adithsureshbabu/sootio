...
// Import cache-store for SQLite/Postgres persistence
...
const SQLITE_SERVICE_KEY = 'mkvcinemas-streams';
const SQLITE_PAGE_PREFIX = 'dlpage:';
const SQLITE_META_PREFIX = 'meta:';
...
async function getDbCached(hashKey) {
    ...
    try {
        ...
        if (updatedAt) {
            ...
            if (age <= PAGE_CACHE_TTL) return cached.data;
        }
    } catch (error) {
...
async function setDbCache(hashKey, data) {
...
async function parseDirectLinkMetadata(url) {
    ...
    const dbCacheKey = `${SQLITE_META_PREFIX}${url}`;
    ...
    // Check db cache
    const dbCached = await getDbCached(dbCacheKey);
    if (dbCached) {
        metadataCache.set(url, { data: dbCached, ts: Date.now() });
        return dbCached;
    }
...
async function extractDownloadOptions(downloadPageUrl) {
    // Check in-memory cache first
    const dbCacheKey = `${SQLITE_PAGE_PREFIX}${downloadPageUrl}`;
    ...
    const dbCached = await getDbCached(dbCacheKey);
    if (dbCached) {
        downloadPageCache.set(downloadPageUrl, { data: dbCached, ts: Date.now() });
        return dbCached;
    }
