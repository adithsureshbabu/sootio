...
const SQLITE_SERVICE_KEY = 'hubcloud-extraction';
const SQLITE_EXTRACTION_PREFIX = 'extract:';
...
const cfCookieCache = new Map(); // domain -> { cookies, userAgent, timestamp }
...
async function cacheCfCookies(domain, cookies, userAgent) {
    if (!domain || !Array.isArray(cookies) || cookies.length === 0 || !userAgent) return;
...
async function fetchWithFlareSolverr(url, headers = {}) {
    ...
    try {
        ...
        if (lower.includes('just a moment') || lower.includes('checking your browser') || lower.includes('cf-browser-verification')) {
            console.log(`[HubDrive] FlareSolverr still blocked for ${url}`);
            return null;
        }
        ...
        if (domain && solution.cookies && solution.userAgent) {
            await cacheCfCookies(domain, solution.cookies, solution.userAgent);
        }
...
async function getSqliteCached(hashKey) {
    if (!SqliteCache.isEnabled()) return null;
    try {
        ...
        if (updatedAt) {
            const age = Date.now() - new Date(updatedAt).getTime();
            if (age <= EXTRACTION_CACHE_TTL) return cached.data;
        }
    } catch (error) {
        console.error(`[HubCloud] Failed to read sqlite cache: ${error.message}`);
    }
    return null;
}

// Helper to write to SQLite cache
async function setSqliteCache(hashKey, data) {
    if (!SqliteCache.isEnabled()) return;
    try {
        await SqliteCache.upsertCachedMagnet({
            ...
            hash: hashKey,
            data,
            releaseKey: 'hubcloud-extraction'
        }, { ttlMs: EXTRACTION_CACHE_TTL });
    } catch (error) {
        console.error(`[HubCloud] Failed to write sqlite cache: ${error.message}`);
    }
}
...
export async function extractHubCloudLinks(url, referer) {
    ...
    const sqliteCacheKey = `${SQLITE_EXTRACTION_PREFIX}${cacheKey}`;
    ...
    // Check SQLite/Postgres cache
    const sqliteCached = await getSqliteCached(sqliteCacheKey);
    if (sqliteCached) {
        console.log(`[HubCloud] Extraction cache hit (db) for: ${url}`);
        extractionCache.set(cacheKey, { data: sqliteCached, ts: Date.now() });
        return sqliteCached;
    }
    ...
    return makeRequest(url, { parseHTML: true, headers })
        .then(async response => {
            ...
            if (isCloudflareChallenge(response.body, response.statusCode)) {
                ...
                console.log(`[HubCloud] Cloudflare challenge detected for ${url}, attempting FlareSolverr bypass...`);
            ...
            try {
                return await Promise.resolve(validButtonResults)
                    ...
                    .then(async (results) => {
                        ...
                        const TRUSTED_SERVERS = ['Pixeldrain', 'FSL V2', 'FSL', 'R2', 'FastDl', 'Cf Worker'];
        ...
        .then(results => {
            // Cache successful results (memory + db)
            if (results && results.length > 0) {
                extractionCache.set(cacheKey, { data: results, ts: Date.now() });
                setSqliteCache(sqliteCacheKey, results);
            }
            return results;
        })
        .finally(() => {
...
export function extractHubDriveLinks(url, depth = 0, signal = null) {
    ...
    return getCachedCfCookies(domain).then(cachedCf => {
        const headers = cachedCf ? { 'User-Agent': cachedCf.userAgent, 'Cookie': cachedCf.cookies } : undefined;
        return makeRequest(url, { parseHTML: true, signal, headers })
            .then(async response => {
            let $ = response.document;
            ...
            if (isCloudflareChallenge(response.body, response.statusCode)) {
                ...
                console.log(`[HubDrive] Cloudflare challenge detected for ${url}, attempting FlareSolverr bypass...`);
