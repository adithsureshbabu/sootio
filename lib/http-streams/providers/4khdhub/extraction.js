...
async function _doFlareSolverrRequest(url, headers = {}) {
    ...
    try {
        ...
        if (!sessionId && shouldUseFlareProxy(url)) {
        ...
        if (domain && solution.cookies) {
            await cacheCfCookies(domain, solution.cookies, solverUserAgent);
        }
...
async function fetchWithFlareSolverr(url, headers = {}) {
    ...
    if (existingLock) {
        ...
        try {
            ...
            // No cookies found after waiting - the previous request failed to get cookies
            // Don't make another FlareSolverr request, just return null
            console.log(`[HubDrive] No cookies cached after waiting for lock on ${domain}, skipping FlareSolverr`);
            return null;
        } catch {
    ...
    if (newLock) {
        ...
        try {
            ...
            console.log(`[HubDrive] No cookies cached after second wait on ${domain}, skipping FlareSolverr`);
            return null;
        } catch {
            // continue
        }
...
export async function extractHubCloudLinks(url, referer) {
    ...
    let activeHeaders = await getCfHeadersForUrl(url);

    return makeRequest(url, { parseHTML: true, headers: activeHeaders || undefined })
        .then(async response => {
            ...
            if (isCloudflareChallenge(response.body, response.statusCode)) {
                ...
                if (flareResponse?.useCachedCookies && flareResponse.cached) {
                    ...
                    const retryResponse = await makeRequest(url, { parseHTML: true, headers: retryHeaders });
                    ...
                    activeHeaders = await getCfHeadersForUrl(url);
            ...
            if (href.includes('hubcloud.one')) {
                ...
                const nextHeaders = await getCfHeadersForUrl(href);
                return makeRequest(href, { parseHTML: true, headers: nextHeaders || activeHeaders || undefined });
            } else {
        ...
        return makeRequest(url, { parseHTML: true, signal, headers })
            ...
            if (isCloudflareChallenge(response.body, response.statusCode)) {
                ...
                if (flareResponse?.useCachedCookies && flareResponse.cached) {
                    ...
                    const retryResponse = await makeRequest(url, { parseHTML: true, signal, headers: retryHeaders });
                    if (retryResponse && !isCloudflareChallenge(retryResponse.body, retryResponse.statusCode)) {
                        $ = retryResponse.document;
