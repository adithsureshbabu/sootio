...
export async function extractHubCloudLinks(url, referer) {
    ...
    return makeRequest(url, { parseHTML: true })
        ...
        .then(async response => {
            ...
            // Process download buttons in smaller batches to reduce memory pressure
            const BUTTON_BATCH_SIZE = 5;
            const allButtonResults = [];

            for (let i = 0; i < buttonsToProcess.length; i += BUTTON_BATCH_SIZE) {
                const batch = buttonsToProcess.slice(i, i + BUTTON_BATCH_SIZE);
                console.log(`Processing button batch ${Math.floor(i / BUTTON_BATCH_SIZE) + 1}/${Math.ceil(buttonsToProcess.length / BUTTON_BATCH_SIZE)}`);

                const buttonPromises = batch.map(async (button, batchIndex) => {
                ...
                const batchResults = await Promise.all(buttonPromises);
                const validBatchResults = batchResults.filter(result => result !== null);
                allButtonResults.push(...validBatchResults);

                // Release memory after each batch
                batchResults.length = 0;
            }

            console.log(`HubCloud extraction completed, found ${allButtonResults.length} valid links`);

            // Sort by priority (highest first)
            allButtonResults.sort((a, b) => (b.priority || 0) - (a.priority || 0));
            console.log(`Sorted links by priority: FSL V2 (100) > FSL (90) > Mega (80) > PixelServer (70) > Others (0)`);

            return Promise.resolve(allButtonResults)
                .then(results => {
                ...
                .then(async (results) => {
                    ...
                    const filteredResults = results.filter(result => {
                        ...
                        if (urlLower.includes('ampproject.org') ||
                            ...
                            (urlLower.match(/[A-Za-z0-9+/=]{100,}/g) !== null)) { // Excessive base64
                    ...
                    // MEMORY OPTIMIZATION: Limit validation batch size and total validations
                    ...
                    // Validate remaining URLs (in batches of 3 for better memory management)
                    const validatedUrls = [];
                    for (let i = 0; i < resultsToValidate.length; i += 3) {
                        const batch = resultsToValidate.slice(i, i + 3);
                        const validationResults = await Promise.all(batch.map(async (result) => {
                            try {
                                ...
                                const validation = await validateSeekableUrl(result.url, { requirePartialContent: true, timeout: 5000 });
                    ...
                    // Clear filtered results to free memory
                    filteredResults.length = 0;
                    resultsToValidate.length = 0;

                    console.log(`Validation complete: ${validatedUrls.length} URLs support 206`);
 ...
 * @returns {Promise<Array>} Array of extracted links
 */
export function extractHubDriveLinks(url, depth = 0) {
            ...
            if (!downloadBtn || downloadBtn.length === 0) {
                ...
                if (hubcloudDownload.length > 0 && (currentUrl.includes('hubcloud.one') || currentUrl.includes('gamerxyt.com'))) {
                    ...
                    if (href) {
                        ...
                        return processHubDriveLink(processedHref, depth);
                    }
                }
                ...
                if (!foundBtn || foundBtn.length === 0) {
                    ...
                    return processHubDriveLink(preferredLink.href, depth);
                }
                ...
                return processHubDriveLink(href, depth);
            }
 ...
 * @returns {Promise<Array>} Array of extracted links
 */
export function processHubDriveLink(href, depth = 0) {
    ...
    // Check if it's FileBee - extract download link
        ...
        return extractFileBeeLinks(href);
    }
    // Check if it's DropGalaxy/dgdrive - extract download link
        ...
        return extractDropGalaxyLinks(href);
    }
    // Check if it's another HubDrive link - recurse with depth tracking
        ...
        return extractHubDriveLinks(href, depth + 1);
    }
    else {
 ...
 * @returns {Promise<Array>} Array of extracted links
 */
export async function extractFileBeeLinks(url) {
 ...
 * @returns {Promise<Array>} Array of extracted links
 */
export async function extractDropGalaxyLinks(url) {
...
export async function processExtractorLinkWithAwait(link, linkNumber) {
    ...
    } else if (linkLower.includes('vcloud.') || linkLower.includes('gdflix')) {
