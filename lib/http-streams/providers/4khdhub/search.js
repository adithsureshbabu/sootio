...
// Cache configuration
const SEARCH_CACHE_TTL = parseInt(process.env.KHDHUB_SEARCH_CACHE_TTL, 10) || 30 * 60 * 1000; // 30 minutes
const CONTENT_CACHE_TTL = parseInt(process.env.KHDHUB_CONTENT_CACHE_TTL, 10) || 10 * 60 * 1000; // 10 minutes
const SQLITE_SERVICE_KEY = '4khdhub';
const SQLITE_SEARCH_PREFIX = 'search:';
const SQLITE_CONTENT_PREFIX = 'content:';

// In-memory hot cache (backed by SQLite)
const searchCache = new Map(); // query -> { data, ts }
const contentCache = new Map(); // url -> { data, ts }

// Helper to get from SQLite cache
async function getSqliteCached(hashKey, ttl) {
    ...
    try {
        ...
        console.error(`[4KHDHub] Failed to read sqlite cache: ${error.message}`);
    }
    return null;
}

// Helper to write to SQLite cache
async function setSqliteCache(hashKey, data, ttlMs) {
    ...
    try {
        await SqliteCache.upsertCachedMagnet({
            ...
            hash: hashKey,
            data,
            releaseKey: '4khdhub-http-streams'
        }, { ttlMs });
    } catch (error) {
        console.error(`[4KHDHub] Failed to write sqlite cache: ${error.message}`);
    }
}
...
export async function scrape4KHDHubSearch(searchQuery) {
    ...
    const sqliteCacheKey = `${SQLITE_SEARCH_PREFIX}${cacheKey}`;
    const cached = searchCache.get(cacheKey);
    if (cached && Date.now() - cached.ts < SEARCH_CACHE_TTL) {
        console.log(`[4KHDHub] Search cache hit (memory) for "${searchQuery}"`);
        return cached.data;
    }

    // Check SQLite cache
    const sqliteCached = await getSqliteCached(sqliteCacheKey, SEARCH_CACHE_TTL);
    if (sqliteCached) {
        console.log(`[4KHDHub] Search cache hit (sqlite) for "${searchQuery}"`);
        searchCache.set(cacheKey, { data: sqliteCached, ts: Date.now() });
        return sqliteCached;
    }
    ...
    try {
        ...
        // Cache the results (memory + SQLite)
        searchCache.set(cacheKey, { data: uniqueResults, ts: Date.now() });
        setSqliteCache(sqliteCacheKey, uniqueResults, SEARCH_CACHE_TTL);
...
export async function loadContent(url) {
    // Check in-memory cache first
    const sqliteCacheKey = `${SQLITE_CONTENT_PREFIX}${url}`;
    const cached = contentCache.get(url);
    if (cached && Date.now() - cached.ts < CONTENT_CACHE_TTL) {
        console.log(`[4KHDHub] Content cache hit (memory) for ${url}`);
        return cached.data;
    }

    // Check SQLite cache
    const sqliteCached = await getSqliteCached(sqliteCacheKey, CONTENT_CACHE_TTL);
    if (sqliteCached) {
        console.log(`[4KHDHub] Content cache hit (sqlite) for ${url}`);
        contentCache.set(url, { data: sqliteCached, ts: Date.now() });
        return sqliteCached;
    }
    ...
    return makeRequest(url, { parseHTML: true })
        .then(response => {
            ...
            // Extract year from title (e.g., "Wicked: For Good (2025)" -> 2025)
            // Look for 4-digit year in parentheses at the end of the title
            const yearMatch = rawTitle.match(/\((\d{4})\)\s*$/);
            const year = yearMatch ? parseInt(yearMatch[1]) : null;
            console.log(`[4KHDHub] Parsed year from title "${rawTitle}": ${year}`);
            ...
            if (isMovie) {
                ...
                contentCache.set(url, { data: content, ts: Date.now() });
                setSqliteCache(sqliteCacheKey, content, CONTENT_CACHE_TTL);
