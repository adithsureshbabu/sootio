...
// Always extract filenames and sizes from drive pages - this is essential for filtering packs/zips
// Only hub pages are now included, so we always need to get the actual file info
...
function isPackFile(filename, size, isEpisodeRequest = false) {
    ...
    if (PACK_KEYWORDS.some(keyword => filenameLower.includes(keyword))) {
        return true;
    }
    ...
    if (isEpisodeRequest && size) {
        ...
        if (sizeMatch) {
            ...
            // Single episodes are rarely larger than 3GB unless they're 4K REMUX
            // But packs are typically 5GB+ for multiple episodes
            if (sizeInGB > 5) {
                console.log(`[HDHub4u] Skipping large file (${size}) as likely pack: ${filename}`);
                return true;
            }
        }
    }
...
export async function getHDHub4uStreams(imdbId, type, season = null, episode = null, prefetchedMeta = null) {
    ...
    try {
        ...
        if (isLazyLoadEnabled()) {
            ...
            // Always extract filenames/sizes from hub pages - this is essential for filtering packs
