...
// Size extraction from hubdrive/hubcloud pages in lazy-load mode (disable with HDHUB4U_EXTRACT_SIZES=false)
const EXTRACT_SIZES_IN_LAZY_MODE = process.env.HDHUB4U_EXTRACT_SIZES !== 'false';
const EXTRACT_FILENAME_TIMEOUT_MS = parseInt(process.env.HDHUB4U_EXTRACT_TIMEOUT_MS, 10) || 1500;
const EXTRACT_FILENAME_CONCURRENCY = parseInt(process.env.HDHUB4U_EXTRACT_CONCURRENCY, 10) || 6;
const EXTRACT_FILENAME_MAX_LINKS = parseInt(process.env.HDHUB4U_EXTRACT_MAX_LINKS, 10) || 6;
    ...
    try {
        ...
        while (matchIndex < maxAttempts && !content) {
            ...
            if ((type === 'series' || type === 'tv') && season) {
                ...
                if (seasonMatch) {
                    titleSeason = parseInt(seasonMatch[1] || seasonMatch[2]);
                    if (titleSeason !== requestedSeason) {
                        console.log(`[HDHub4u] Season mismatch in title: found Season ${titleSeason}, requested Season ${requestedSeason} - "${candidateContent.title}"`);
                        matchIndex++;
                        continue;
                    }
                }
                const titleSeasonMatches = titleSeason === requestedSeason;
        ...
        if (isLazyLoadEnabled()) {
            ...
            // Optionally extract filenames/sizes from hub pages
            let filenameMap = new Map();
            const needsSizeLookup = EXTRACT_SIZES_IN_LAZY_MODE && limited.some(link => !link.size);
            if (EXTRACT_FILENAMES_IN_LAZY_MODE || needsSizeLookup) {
                try {
                    let targets = EXTRACT_FILENAMES_IN_LAZY_MODE
                        ? limited
                        : limited.filter(link => !link.size);

                    if (EXTRACT_FILENAME_MAX_LINKS > 0) {
                        targets = targets.slice(0, EXTRACT_FILENAME_MAX_LINKS);
                    }
            ...
            const previewStreams = limited.map(link => {
                ...
                const label = EXTRACT_FILENAMES_IN_LAZY_MODE
                    ? (extractedInfo?.filename || link.label || 'HDHub4u Stream')
                    : (link.label || extractedInfo?.filename || 'HDHub4u Stream');
