...
const BASE_HOSTS = BASE_URLS
    ...
    .filter(Boolean);
const SITEMAP_INDEX_URLS = BASE_URLS.map(base => `${base}/sitemap.xml`);
...
const SQLITE_SITEMAP_ENTRIES_KEY = `sitemap:entries:${BASE_HOSTS.join(',') || 'default'}`;
const SQLITE_SITEMAP_RELEASE_KEY = 'hdhub4u-sitemap';
...
async function fetchSitemapList() {
    ...
    try {
        const sitemapResults = await Promise.all(
            SITEMAP_INDEX_URLS.map(async (indexUrl) => {
                try {
                    ...
                    console.error(`[HDHub4u] Failed to fetch sitemap index ${indexUrl}: ${error.message}`);
                    return [];
                }
            })
        );

        const postSitemaps = sitemapResults.flat();
        const unique = [];
        const seen = new Set();
...
async function buildFuseIndex() {
    ...
    buildingFusePromise = (async () => {
        try {
            const sqliteEntries = await getSqliteCachedSitemapEntries();
            if (sqliteEntries && sqliteEntries.length > 0) {
                console.log(`[HDHub4u] Loaded ${sqliteEntries.length} sitemap entries from SQLite cache`);
                fuseIndex = new Fuse(sqliteEntries, {
                    includeScore: true,
                    threshold: 0.4,
            ...
            await cacheSitemapEntries(allEntries);
...
function normalizeLink(href, baseUrl = BASE_URLS[0] || DEFAULT_BASE_URLS[0]) {
    if (!href) return null;
    try {
...
function shouldIncludeLink(url) {
    ...
    try {
        const urlObj = new URL(url);
        if (BASE_HOSTS.includes(urlObj.hostname.toLowerCase())) {
            return false;
        }
    } catch {
...
async function getSqliteCachedSitemapEntries() {
    if (!SqliteCache.isEnabled()) {
        return null;
    }
    try {
        const cached = await SqliteCache.getCachedRecord(SQLITE_SERVICE_KEY, SQLITE_SITEMAP_ENTRIES_KEY);
        if (cached?.data && Array.isArray(cached.data)) {
            return cached.data;
        }
    } catch (error) {
        console.error(`[HDHub4u] Failed to read sqlite sitemap cache: ${error.message}`);
    }
    return null;
}

async function cacheSitemapEntries(entries) {
    if (!SqliteCache.isEnabled() || !Array.isArray(entries) || entries.length === 0) {
        return;
    }
    try {
        await SqliteCache.upsertCachedMagnet({
            service: SQLITE_SERVICE_KEY,
            hash: SQLITE_SITEMAP_ENTRIES_KEY,
            fileName: 'hdhub4u-sitemap-entries',
            data: entries,
            releaseKey: SQLITE_SITEMAP_RELEASE_KEY
        }, { ttlMs: SITEMAP_CACHE_TTL });
    } catch (error) {
        console.error(`[HDHub4u] Failed to cache sitemap entries in sqlite: ${error.message}`);
    }
}
