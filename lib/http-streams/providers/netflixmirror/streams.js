...
import { buildNetflixMirrorProxyUrl } from './proxy.js';
...
export async function getNetflixMirrorStreams(tmdbId, type, season = null, episode = null, config = {}, prefetchedMeta = null) {
    try {
        ...
        // Load content details (pass search info so movies can skip broken post.php)
        const content = await loadNetflixMirrorContent(bestMatch.id, null, bestMatch.title, bestMatch.type || (type === 'series' ? 'series' : 'movie'));
        if (!content) {
        ...
        const streams = [];
        const streamHeaders = playlist.requestHeaders || getStreamHeaders();

        for (const source of playlist.sources) {
            const quality = normalizeQuality(source.label);
            const absoluteSourceUrl = source.url?.startsWith('http')
                ? source.url
                : new URL(source.url || '', STREAM_URL).toString();
            const resolverBase = (config?.host && String(config.host).startsWith('http')) ? String(config.host) : '';
            const streamUrl = resolverBase
                ? buildNetflixMirrorProxyUrl(absoluteSourceUrl, absoluteSourceUrl, resolverBase)
                : absoluteSourceUrl;
