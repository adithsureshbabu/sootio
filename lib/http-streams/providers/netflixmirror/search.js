...
export async function getNetflixMirrorPlaylist(id, title, signal = null) {
    ...
    try {
        ...
        const playlist = {
            sources: [],
            subtitles: [],
            items: [] // keep original shape for clients that expect per-item data
        };

        for (const item of data) {
            const normalizedItem = {
                title: item.title,
                image: item.image ? new URL(item.image, IMG_CDN).toString() : undefined,
                sources: [],
                tracks: []
            };
            ...
            if (item.sources && Array.isArray(item.sources)) {
                for (const source of item.sources) {
                    ...
                    try {
                        streamUrl = new URL(streamUrl, STREAM_URL).toString();
                    } catch {
                        streamUrl = streamUrl.startsWith('http') ? streamUrl : `${STREAM_URL}${streamUrl}`;
                    }
            ...
            if (item.tracks && Array.isArray(item.tracks)) {
                for (const track of item.tracks) {
                    if (track.kind === 'captions' && track.file) {
                        ...
                        try {
                            subUrl = subUrl.startsWith('//')
                                ? `https:${subUrl}`
                                : new URL(subUrl, STREAM_URL).toString();
                        } catch {
                            subUrl = subUrl.startsWith('http') ? subUrl : `https:${subUrl}`;
                        }
            ...
            playlist.items.push(normalizedItem);
        }

        // If the server returned "unknown" tokens, retry once with a fresh cookie
        const unknownSources = playlist.items.every(it =>
            it.sources.every(src => (src.file || '').includes('unknown'))
        );
        if (unknownSources) {
            console.log('[NetflixMirror] Playlist returned unknown tokens, retrying with fresh cookie...');
            resetCachedCookie();
            return await getNetflixMirrorPlaylist(id, title, signal);
        }
...
/**
 * Get fully authenticated headers for proxying playlists/segments.
 * Falls back to a minimal header set if the bypass cookie is unavailable.
 */
export async function getAuthStreamHeaders() {
    const baseHeaders = getStreamHeaders();

    // Test hook to avoid network calls
    if (process.env.NETFLIXMIRROR_TEST_COOKIE) {
        return {
            ...baseHeaders,
            Cookie: `t_hash_t=${process.env.NETFLIXMIRROR_TEST_COOKIE}; ott=nf; hd=on`
        };
    }
    ...
    try {
        ...
        console.log(`[NetflixMirror] Failed to get auth headers, using fallback: ${err.message}`);
        return baseHeaders;
    }
}
