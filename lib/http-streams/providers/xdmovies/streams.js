...
import { resolveXDMoviesProtectedUrl } from './protector.js';
...
async function resolveXDMoviesLink(url) {
    ...
    if (url.includes(XDMOVIES_LINK_HOST) || url.includes('link.xdmovies')) {
        try {
            const finalUrl = await resolveXDMoviesProtectedUrl(url);
            if (finalUrl) {
                console.log(`[XDMovies] Resolved protector URL to: ${finalUrl}`);
                return finalUrl;
            }
        } catch (error) {
