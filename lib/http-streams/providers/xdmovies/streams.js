...
// XDMovies uses link.xdmovies.wtf as a redirector to a link protector then HubCloud
const XDMOVIES_LINK_HOST = 'link.xdmovies.wtf';
...
/**
 * Resolve XDMovies link through link protector to get HubCloud URL
 * Flow: link.xdmovies.wtf/download/X → protector/r/CODE → POST /api/session → /go/sessionId?t=token → hubcloud URL
 * @param {string} url - XDMovies link (link.xdmovies.wtf)
...
async function resolveXDMoviesLink(url) {
    ...
    // If it's an xdmovies link, resolve through link protector
    if (url.includes(XDMOVIES_LINK_HOST) || url.includes('link.xdmovies')) {
        try {
            const { makeRequest } = await import('../../utils/http.js');

            // Step 1: Follow redirect from link.xdmovies.wtf to link protector
            const step1 = await makeRequest(url, { allowRedirects: false, timeout: 8000 });
            const protectorUrl = step1.headers?.location;
            if (!protectorUrl) {
                console.log(`[XDMovies] No redirect from ${url}`);
                return url;
            }
            console.log(`[XDMovies] Link protector redirect: ${url} -> ${protectorUrl}`);

            // Step 2: Extract code from protector URL path and POST to /api/session
            const protectorParsed = new URL(protectorUrl);
            const code = protectorParsed.pathname.split('/').pop();
            const protectorBase = `${protectorParsed.protocol}//${protectorParsed.host}`;

            const sessionResponse = await makeRequest(`${protectorBase}/api/session`, {
                method: 'POST',
            ...
            try {
                ...
                console.log(`[XDMovies] Failed to parse session response`);
                return url;
            }

            if (!sessionData.sessionId || !sessionData.token) {
                console.log(`[XDMovies] Invalid session response: ${JSON.stringify(sessionData)}`);
                return url;
            }

            // Step 3: Follow /go/{sessionId}?t={token} to get final hubcloud URL
            const goUrl = `${protectorBase}/go/${sessionData.sessionId}?t=${encodeURIComponent(sessionData.token)}`;
            const step3 = await makeRequest(goUrl, { allowRedirects: false, timeout: 8000 });
            const finalUrl = step3.headers?.location;

            if (finalUrl) {
                console.log(`[XDMovies] Resolved to final URL: ${finalUrl}`);
                return finalUrl;
            }

            console.log(`[XDMovies] No final redirect from /go/ endpoint`);
            return url;
        } catch (error) {
            console.log(`[XDMovies] Failed to resolve link protector for ${url}: ${error.message}`);
        }
    }
