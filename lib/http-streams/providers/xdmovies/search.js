...
const BASE_URL = process.env.XDMOVIES_BASE_URL || 'https://xdmovies.site';
...
const SQLITE_SERVICE_KEY = 'xdmovies';
...
export async function searchXDMovies(query, limit = 15) {
    ...
    // Check SQLite cache
    const sqliteCached = await getSqliteCachedSearch(cacheKey);
    if (sqliteCached) {
        searchCache.set(cacheKey, { fetchedAt: now, data: sqliteCached });
        console.log(`[XDMovies] Loaded search results from SQLite cache for "${query}"`);
        return sqliteCached.slice(0, limit);
    }
...
export async function loadXDMoviesContent(url) {
    ...
    if (sqliteCached) {
        pageCache.set(url, { fetchedAt: now, data: sqliteCached });
        console.log(`[XDMovies] Loaded content from SQLite cache for ${url}`);
        return sqliteCached;
    }
...
async function getSqliteCachedSearch(cacheKey) {
    if (!SqliteCache.isEnabled()) return null;
    try {
        const cached = await SqliteCache.getCachedRecord(SQLITE_SERVICE_KEY, cacheKey);
        ...
        console.error(`[XDMovies] SQLite search cache read error: ${error.message}`);
    }
    return null;
}

async function cacheSearchResults(cacheKey, data) {
    if (!SqliteCache.isEnabled()) return;
    try {
        await SqliteCache.upsertCachedMagnet({
            service: SQLITE_SERVICE_KEY,
            hash: cacheKey,
            fileName: 'xdmovies-search',
            data,
            releaseKey: 'xdmovies-search'
        }, { ttlMs: SEARCH_CACHE_TTL });
    } catch (error) {
        console.error(`[XDMovies] SQLite search cache write error: ${error.message}`);
    }
}
...
async function getSqliteCachedPage(url) {
    ...
    try {
        ...
        console.error(`[XDMovies] SQLite page cache read error: ${error.message}`);
    }
    return null;
}
...
async function cachePageResult(url, data) {
    ...
    try {
        await SqliteCache.upsertCachedMagnet({
            ...
            data,
            releaseKey: 'xdmovies-page'
        }, { ttlMs: PAGE_CACHE_TTL });
    } catch (error) {
        console.error(`[XDMovies] SQLite page cache write error: ${error.message}`);
    }
}
