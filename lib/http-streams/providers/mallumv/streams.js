...
const SQLITE_SERVICE_KEY = 'mallumv';
...
async function setDbCache(hashKey, data, ttlMs) {
    ...
    try {
        await SqliteCache.upsertCachedMagnet({
            ...
            hash: hashKey,
            data,
            releaseKey: 'mallumv-http-streams'
        }, { ttlMs });
    } catch (error) {
...
async function loadMalluMvContent(detailUrl) {
    // Check in-memory cache first
    const dbCacheKey = `${SQLITE_CONTENT_PREFIX}${detailUrl}`;
    ...
    if (dbCached) {
        console.log(`[${PROVIDER}] Content cache hit (db) for ${detailUrl}`);
        contentCache.set(detailUrl, { data: dbCached, ts: Date.now() });
        return dbCached;
    }
    ...
    try {
        ...
        setDbCache(dbCacheKey, downloads, PAGE_CACHE_TTL);
