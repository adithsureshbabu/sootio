...
export async function scrapeMkvDramaSearch(query, signal = null) {
    ...
    if (cached && Date.now() - cached.ts < SEARCH_CACHE_TTL) {
        console.log(`[MKVDrama] Search cache hit (memory) for "${query}"`);
        return cached.data;
    }
    ...
    try {
        ...
        // Cache even fallback results (memory + db)
        if (results.length > 0) {
            searchCache.set(cacheKey, { data: results, ts: Date.now() });
            setDbCache(dbCacheKey, results, SEARCH_CACHE_TTL);
        }
...
export async function loadMkvDramaContent(postUrl, signal = null, options = {}) {
    ...
    if (depth === 0) {
        ...
        if (cached && Date.now() - cached.ts < CONTENT_CACHE_TTL) {
            console.log(`[MKVDrama] Content cache hit (memory) for ${postUrl}`);
            return cached.data;
        }
    ...
    try {
        ...
        // Cache the result (only for top-level calls) - memory + db
        if (depth === 0) {
