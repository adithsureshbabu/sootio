...
// ouo.io is the primary short link service, oii.la is an alternative that appears on some pages
const OUO_HOSTS = ['ouo.io', 'oii.la'];
...
// Cache configuration
const SEARCH_CACHE_TTL = parseInt(process.env.MKVDRAMA_SEARCH_CACHE_TTL, 10) || 30 * 60 * 1000; // 30 minutes
const CONTENT_CACHE_TTL = parseInt(process.env.MKVDRAMA_CONTENT_CACHE_TTL, 10) || 10 * 60 * 1000; // 10 minutes
const CF_COOKIE_CACHE_TTL = parseInt(process.env.MKVDRAMA_CF_COOKIE_TTL, 10) || 25 * 60 * 1000; // 25 minutes (CF cookies typically last 30min)
const SQLITE_SERVICE_KEY = 'mkvdrama';
const SQLITE_SEARCH_PREFIX = 'search:';
const SQLITE_CONTENT_PREFIX = 'content:';
...
async function cacheCfCookies(domain, cookies, userAgent) {
    ...
    const relevant = cookies.filter(cookie =>
        ...
        cookie.name === 'cf_chl_seq' ||
        cookie.name === '__cf_bm'
    );
    if (!relevant.length) return;

    const cookieString = relevant.map(cookie => `${cookie.name}=${cookie.value}`).join('; ');
    const cookieData = {
    ...
    try {
        ...
        console.log(`[MKVDrama] Cached CF cookies for ${domain} (memory + DB): ${relevant.map(c => c.name).join(', ')}`);
    } catch (error) {
        console.error(`[MKVDrama] Failed to persist CF cookie to DB: ${error.message}`);
        console.log(`[MKVDrama] Cached CF cookies for ${domain} (memory only): ${relevant.map(c => c.name).join(', ')}`);
    }
}
...
async function fetchPage(url, signal = null) {
    ...
    try {
        ...
        // Only treat as "blocked by Cloudflare" if we see actual CF challenge page
        // Status 403/429 alone doesn't mean CF blocked - site might just return 403 for "not found"
        const isCloudflareBlocked = isCloudflare || (response.statusCode === 429 && !body);

        if (isCloudflareBlocked && usedCachedCookie) {
            console.log(`[MKVDrama] Cached CF cookies denied for ${url}, clearing`);
            clearCachedCfCookies(domain);
        }

        if (isCloudflareBlocked && FLARESOLVERR_URL) {
...
export async function scrapeMkvDramaSearch(query, signal = null) {
    ...
    if (Array.isArray(dbCached) && dbCached.length > 0) {
        console.log(`[MKVDrama] Search cache hit (db) for "${query}"`);
        return dbCached;
    }
    ...
    try {
        ...
        // Cache even fallback results (db only)
        if (results.length > 0) {
            setDbCache(dbCacheKey, results, SEARCH_CACHE_TTL);
        }
        return results;
...
export async function loadMkvDramaContent(postUrl, signal = null, options = {}) {
    ...
    const dbCacheKey = `${SQLITE_CONTENT_PREFIX}${postUrl}`;

    // Check cache first (only for non-nested calls to avoid stale episode data)
    if (depth === 0) {
        // Check db cache
        const dbCached = await getDbCached(dbCacheKey, CONTENT_CACHE_TTL);
        if (dbCached?.downloadLinks?.length > 0) {
            console.log(`[MKVDrama] Content cache hit (db) for ${postUrl}`);
            return dbCached;
        }
    }
    ...
    try {
        ...
        // Cache the result (only for top-level calls) - db only
        if (depth === 0 && downloadLinks.length > 0) {
            setDbCache(dbCacheKey, result, CONTENT_CACHE_TTL);
        }
