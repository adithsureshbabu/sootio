 ...
 * Only checks for actual challenge page indicators, not cookie names or generic mentions
 */
function isCloudflareChallenge(body = '', statusCode = null) {
    // Don't just check status codes - 403/503 can happen for other reasons
    const lower = body.toLowerCase();

    // These are specific indicators of an actual Cloudflare challenge/interstitial page
    return (lower.includes('just a moment') && lower.includes('cloudflare')) ||
        lower.includes('checking your browser') ||
        lower.includes('cf-browser-verification') ||
        lower.includes('cf-turnstile') ||
        lower.includes('verify_turnstile') ||
        (lower.includes('challenge-platform') && lower.includes('ray id'));
}
...
/**
 * Fetch a page - tries direct request first, only uses FlareSolverr if Cloudflare blocks us
 */
async function fetchPage(url, signal = null) {
    ...
    // Strategy: Try direct requests first, only use FlareSolverr if Cloudflare actually blocks us

    // 1. Try with cached cf_clearance cookie first (if available)
    ...
    // 2. Try direct request without cookie
    if (!cloudflareDetected) {
        try {
            ...
            console.log(`[MKVDrama] Cloudflare challenge detected (status: ${response.statusCode})`);
            cloudflareDetected = true;
        } catch (error) {
            // Don't assume Cloudflare on network errors - could be timeout, DNS, etc.
            console.error(`[MKVDrama] Direct request failed for ${url}: ${error.message}`);
            // Only set cloudflareDetected if error message suggests Cloudflare
    ...
    // 3. Only use FlareSolverr if Cloudflare was actually detected
    if (!cloudflareDetected) {
        console.log(`[MKVDrama] No Cloudflare detected, not using FlareSolverr for ${url}`);
        return null;
    }
    ...
    console.log(`[MKVDrama] Cloudflare detected, using FlareSolverr for ${url}`);
