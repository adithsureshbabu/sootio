...
export function validateUrl(url, options = {}) {
    ...
    const timeout = typeof options.timeout === 'number'
        ? options.timeout
        : (parseInt(process.env.VALIDATION_TIMEOUT) || 8000); // Default 8 seconds
 ...
 * @returns {Promise<{isValid: boolean, filename: string|null, statusCode: number|null, contentLength?: number|null}>} Validation details
 */
export function validateSeekableUrl(url, options = {}) {
    ...
    return new Promise((resolve) => {
        try {
            const urlObj = new URL(url);
            const isGoFileHost = urlObj.hostname.includes('gofile.io');
            const isGoFileDownload = isGoFileHost && (
                urlObj.hostname.startsWith('store-') ||
                urlObj.pathname.includes('/download/')
            );
            if (isGoFileDownload && requirePartialContent) {
                let filename = null;
                try {
                ...
                console.log(`[4KHDHub] Skipping 206 validation for GoFile direct download: ${url}`);
                resolve({ isValid: true, filename, statusCode: 200, contentLength: null });
                return;
            }
            ...
            const req = protocol.request(url, options, (res) => {
                // Follow redirects for GoFile URLs (they return 302 to actual file server)
                if ((res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 307 || res.statusCode === 308) && res.headers.location) {
                    const isGoFileRedirect = url.includes('gofile.io');
                    if (isGoFileRedirect) {
                        const redirectUrl = new URL(res.headers.location, url).toString();
                        console.log(`[4KHDHub] Following GoFile redirect: ${redirectUrl.substring(0, 80)}...`);
                        res.destroy();
                        // Recursively validate the redirect target
                        const redirectTimeout = Math.min(timeout, Math.max(500, timeout - 2000));
                        resolve(validateSeekableUrl(redirectUrl, { requirePartialContent, timeout: redirectTimeout }));
                        return;
                    }
                }
