    ...
    try {
        ...
        if (response.data?.status === 'ok' && response.data?.solution?.response) {
            ...
            if (cfClearance && userAgent) {
                // Verify the cookie actually works before saving
                const isValid = await verifyCookie(baseUrl, cfClearance, userAgent, timeout, logPrefix);
                if (isValid) {
                    const saved = await saveCookie(domain, cfClearance, userAgent);
                    if (saved) {
                        console.log(`[${logPrefix} SCRAPER] ExtTo saved verified cf_clearance cookie for ${domain}`);
                    } else {
                        console.log(`[${logPrefix} SCRAPER] ExtTo verified cf_clearance cookie but cache backend rejected save`);
                    }
                } else {
                    console.log(`[${logPrefix} SCRAPER] ExtTo cookie from FlareSolverr failed verification - not saving`);
                }
            } else {
...
export async function searchExtTo(query, signal, logPrefix, config) {
    ...
    const scrapePromise = (async () => {
        ...
        for (let page = 1; page <= maxPages; page++) {
            ...
            // 3. Only use FlareSolverr if curl failed (Cloudflare is blocking us)
            if (!html && flareSolverrUrl) {
                if (page === 1 || !sessionId) {
                    ...
                    console.log(`[${logPrefix} SCRAPER] ${scraperName} curl blocked by Cloudflare, using FlareSolverr at ${flareSolverrUrl}${sessionId ? ' (with session)' : ''}`);
                }
                const result = await fetchWithFlareSolverr(searchUrl, flareSolverrUrl, timeout, logPrefix, sessionId, flareLimiter, signal);
                html = result.html;
