...
async function getOrCreateSession(flareSolverrUrl, domain, logPrefix) {
    ...
    // Create new session
    try {
        const sessionId = `extto_${domain}_${Date.now()}`;
...
export async function searchExtTo(query, signal, logPrefix, config) {
    ...
    const scrapePromise = (async () => {
        ...
        for (let page = 1; page <= maxPages; page++) {
            ...
            // If this page had fewer results than expected, no more pages
            if (pageResults.length < 50) {
                console.log(`[${logPrefix} SCRAPER] ${scraperName} page ${page} had ${pageResults.length} results, stopping pagination`);
                break;
            }
        }
