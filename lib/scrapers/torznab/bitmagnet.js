...
const axiosWithProxy = axios.create(debridProxyManager.getScraperAxiosConfig('bitmagnet'));
...
export async function searchBitmagnet(query, signal, logPrefix, config) {
    ...
    try {
        ...
        if (!base || /YOUR_BITMAGNET/i.test(base)) {
            console.error(`[${logPrefix} SCRAPER] ${scraperName} BITMAGNET_URL not configured or placeholder, skipping`);
            return [];
        }
