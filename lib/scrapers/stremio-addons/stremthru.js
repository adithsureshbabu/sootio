...
const axiosWithProxy = axios.create(debridProxyManager.getScraperAxiosConfig('stremthru'));

function isAbortLikeError(error) {
    if (!error) return false;
    if (axios.isCancel?.(error)) return true;
    if (error?.code === 'ERR_CANCELED' || error?.code === 'ERR_ABORTED') return true;
    if (error?.name === 'AbortError') return true;
    const message = String(error?.message || '').toLowerCase();
    return message === 'canceled' || message === 'cancelled' || message.includes('aborted');
}
...
export async function searchStremthru(mediaType, mediaId, signal, season, episode, logPrefix, config) {
    ...
    // Debug: Log if debrid credentials are present for Torz API
    console.log(`[${logPrefix} STREMTHRU] Config check - DEBRID_SERVICE: ${config?.DEBRID_SERVICE || 'NOT SET'}, DEBRID_TOKEN: ${config?.DEBRID_TOKEN ? 'SET' : 'NOT SET'}, mediaType: ${mediaType}, mediaId: ${mediaId}`);
    ...
    try {
        ...
        try {
            ...
            if (isAbortLikeError(torznabError)) {
                console.log(`[${logPrefix} STREMTHRU] Torznab API request canceled`);
            } else {
        ...
        if (config?.DEBRID_TOKEN && config?.DEBRID_SERVICE) {
            try {
                ...
                console.log(`[${logPrefix} STREMTHRU] Calling Torz API for ${mediaType}/${stremId} with service ${config.DEBRID_SERVICE}`);
                ...
                console.log(`[${logPrefix} STREMTHRU] Torz API returned ${rawTorzResults?.length || 0} raw results`);

                // Mark these as coming from Torz source and ensure proper format
                torzStyleResults = (rawTorzResults || []).map(t => ({
                ...
                console.log(`[${logPrefix} STREMTHRU] After filtering for valid InfoHash: ${torzStyleResults.length} results`);
            } catch (torzError) {
                console.error(`[${logPrefix} STREMTHRU] Torz-style API error:`, torzError.message);
                // Continue with torznab results only
            }
        } else {
            console.log(`[${logPrefix} STREMTHRU] Skipping Torz API - missing DEBRID_TOKEN or DEBRID_SERVICE`);
        }
        ...
        if (config?.DEBRID_TOKEN && config?.DEBRID_SERVICE) {
            ...
            try {
                ...
                // Ignore cancellations - these are expected when early-return aborts remaining scrapers
                if (isAbortLikeError(torzError)) {
            ...
            try {
                // Use the full combined set before deduplication so we don't drop any hashes
                const allHashes = combinedResults.map(r => r.InfoHash?.toLowerCase()).filter(Boolean);
                if (allHashes.length > 0) {
                    ...
                    if (cachedHashesFromSqlite.size > 0) {
                        ...
                        // Add SQLite results that aren't already in the working set (avoid hash duplicates only)
                        const existingHashes = new Set(results.map(r => r.InfoHash?.toLowerCase()).filter(Boolean));
            ...
            if (results.length > 0) {
                // Deduplicate only by hash to keep full Torz list intact
                const dedupedByHash = [];
                const seen = new Set();
                for (const result of results) {
                    ...
                    seen.add(hash);
                    dedupedByHash.push(result);
                }

                console.log(`[${logPrefix} STREMTHRU] Returning ${dedupedByHash.length} combined cached results (from Torz direct API + SQLite cache, hash-deduped only)`);
