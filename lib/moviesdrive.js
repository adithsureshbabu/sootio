...
async function extractMoviePageLinks(movieUrl) {
  ...
  try {
    ...
    const $ = response.document;
    const pageText = $('main').text() || $.text();
    const languages = detectLanguagesFromTitle(pageText);
  ...
  try {
    ...
    if (isLazyLoadEnabled()) {
      ...
      const previewStreams = (await Promise.all(moviePageLinks.map(async (link) => {
        ...
        if (/mdrive\.today\/archives\//i.test(link.url)) {
          try {
            const hostLinks = await extractMDriveLinks(link.url);
            const hostCandidate = hostLinks.find(h =>
              h.url &&
              (/hubcloud|gdflix|filesdl/i.test(h.url) || h.type === 'filesdl')
            ) || hostLinks[0];
        ...
        const languages = link.languages && link.languages.length ? link.languages : detectLanguagesFromTitle(link.displayTitle || link.title || '');
        const langFlags = renderLanguageFlags(languages);

        return {
          name: `[HS+] Sootio\n${resolutionLabel}`,
          title: `${link.displayTitle || link.title || 'MoviesDrive Stream'}${langFlags}\n💾 ${link.size || 'Unknown'} | MoviesDrive`,
          ...
          .map(async (mdriveLink) => {
            try {
              ...
              if (mdriveLink.type === 'filesdl') {
                ...
                const languages = detectLanguagesFromTitle(mdriveLink.title || mediaInfo.title);
                const langFlags = renderLanguageFlags(languages);
