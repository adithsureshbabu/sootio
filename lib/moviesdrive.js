...
function sanitizeForLanguageDetection(text = '') {
  ...
  // Remove common site labels and bracketed tags
  cleaned = cleaned
    .replace(/\[(?:[^\]]+)\]/g, ' ')
...
function selectHostLinkForEpisode(hostLinks = [], episode = null, { singleEpisodePage = false } = {}) {
  ...
  const isHub = (h) => h?.url && /hubcloud|hubdrive/i.test(h.url);
  const isGdFlix = (h) => h?.url && /gdflix/i.test(h.url);
        ...
        if (/mdrive\.today\/archives\//i.test(link.url)) {
          try {
            ...
            if (hostCandidate?.url) {
              ...
              if (EXTRACT_FILENAMES_IN_LAZY_MODE && /hubcloud|hubdrive/i.test(hostCandidate.url)) {
                try {
