...
async function getMoviesDriveStreams(imdbId, tmdbId, mediaType = 'movie', season = null, episode = null, config = {}) {
  ...
  try {
    ...
    if (isLazyLoadEnabled()) {
      ...
      const previewStreams = moviePageLinks.map((link) => {
        ...
        const languages = detectLanguagesFromTitle(link.title || '');
        const langFlags = renderLanguageFlags(languages);

        return {
          name: `[HS+] Sootio\n${resolutionLabel}`,
          title: `${link.title || 'MoviesDrive Stream'}${langFlags}\n💾 ${link.size || 'Unknown'} | MoviesDrive`,
          url: link.url, // MDrive page URL - will be resolved on click
