...
// --- Domain Management ---
const DEFAULT_MOVIESDRIVE_DOMAIN = (process.env.MOVIESDRIVE_BASE_URL || 'https://new1.moviesdrives.my/')
  .trim()
  .replace(/\/+$/, '');
...
async function getMoviesDriveDomain() {
  ...
  // Prefer the configured default domain before consulting external domain feeds.
  // This avoids stale domain APIs causing unnecessary delays.
  try {
    moviesDriveDomain = await validateMoviesDriveDomain(DEFAULT_MOVIESDRIVE_DOMAIN, DEFAULT_TIMEOUT);
    domainCacheTimestamp = Date.now();
    return moviesDriveDomain;
  } catch (primaryError) {
    console.warn(`[MoviesDrive] Primary domain check failed for ${DEFAULT_MOVIESDRIVE_DOMAIN}:`, primaryError.message);
  }

  // If primary domain fails, try the public landing page CTA ("Explore Movies")
  // which usually redirects to the current active MoviesDrive host.
  const discoveredDomain = await getDomainFromDiscoveryPage(DEFAULT_TIMEOUT);
  if (discoveredDomain) {
    moviesDriveDomain = discoveredDomain;
    domainCacheTimestamp = Date.now();
    return moviesDriveDomain;
  }
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (response && response.data && (response.data.MoviesDrive || response.data.moviesdrive)) {
        ...
        try {
          moviesDriveDomain = await validateMoviesDriveDomain(newDomain, DEFAULT_TIMEOUT);
          domainCacheTimestamp = Date.now();
  ...
  for (const fallbackDomain of FALLBACK_DOMAINS) {
    try {
      ...
      moviesDriveDomain = await validateMoviesDriveDomain(fallbackDomain, DEFAULT_TIMEOUT);
      domainCacheTimestamp = Date.now();
