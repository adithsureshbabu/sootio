...
const normalizeServiceName = (serviceName = '') => serviceName.toLowerCase().replace(/[.-]/g, '');
...
class DebridProxyManager {
    ...
    /**
     * Scraper-aware wrapper that falls back to the legacy global scrapers config
     */
    getScraperAxiosConfig(scraperName, baseConfig = {}) {
        const normalizedScraper = normalizeServiceName(scraperName);
        const hasCustomProxy = normalizedScraper in this.perServiceProxies;
        const hasCustomToggle = normalizedScraper in this.proxyConfig.services;
        const hasScraperDefault = ('scrapers' in this.perServiceProxies) || this.proxyConfig.services.scrapers === true;

        if (hasCustomProxy || hasCustomToggle) {
            return this.getAxiosConfig(normalizedScraper, baseConfig);
        }

        if (hasScraperDefault) {
            return this.getAxiosConfig('scrapers', baseConfig);
        }
