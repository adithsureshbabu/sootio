...
function setupCleanupJob() {
  ...
  cleanupIntervalId = setInterval(async () => {
    try {
      ...
      // Also clean up old HTTP streams entries based on updated_at
      await runHttpStreamsCleanup();
    } catch (error) {
...
// Internal helper to run HTTP streams cleanup
async function runHttpStreamsCleanup() {
  ...
  try {
    ...
    if (deletedCount > 0) {
      console.log(`[POSTGRES CACHE] HTTP streams cleanup: removed ${deletedCount} entries older than ${ttlDays} days in ${duration}ms`);
    } else if (debug) {
      console.log(`[POSTGRES CACHE] HTTP streams cleanup: no old entries to remove (checked in ${duration}ms)`);
    }
  } catch (error) {
    console.error(`[POSTGRES CACHE] Error in HTTP streams cleanup: ${error.message}`);
  }
}

export async function upsertCachedMagnet(record, options = {}) {
  if (!isEnabled()) {
    console.log(`[POSTGRES CACHE] Cache is not enabled, skipping upsert for hash ${record.hash}`);
    return false;
  }
  ...
  try {
    ...
    if (!service || !hash) {
      console.log(`[POSTGRES CACHE] Invalid service (${service}) or hash (${hash}) for upsert`);
      return false;
    }
    ...
    console.error(`[POSTGRES CACHE] Error upserting magnet record: ${error.message}`);
    return false;
  }
}
...
export async function upsertCachedMagnets(records, options = {}) {
  ...
  try {
    ...
    for (const record of records) {
      ...
      const hash = String(record.hash || '').toLowerCase();
      if (!service || !hash) {
        if (debug) console.log(`[POSTGRES CACHE] Skipping invalid record: service=${record.service}, hash=${record.hash}`);
        continue;
      }
  ...
  const duration = Date.now() - startTime;
  if (debug) {
    console.log(`[POSTGRES CACHE] Individual upserts completed in ${duration}ms for ${successCount}/${records.length} records`);
  }
