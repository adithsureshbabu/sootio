...
const CONFIG = {
    ...
    PENALTIES: {
        TIMEOUT: 5,           // 5 minutes for timeouts (not used)
    ...
    recordFailure(scraperName, errorType, responseTime = 0, errorMessage = '') {
        ...
        switch (errorType) {
            case 'timeout':
                ...
                // No penalty for timeouts - they're often transient network issues
                // and don't indicate a problem with the scraper itself
                break;
            case 'rate_limit':
            case '429':
                data.rateLimitCount++;
