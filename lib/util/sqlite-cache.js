...
export function isEnabled() {
  if (config.CACHE_BACKEND === 'postgres') return false;
...
export async function deleteCachedHash(service, hash) {
  if (!isEnabled()) {
    console.log(`[SQLITE CACHE] SQLite cache is not enabled, skipping delete for ${hash}`);
  ...
  try {
    ...
    db.prepare(`
      DELETE FROM cache
      WHERE service = ? AND hash = ?
    `).run(serviceKey, hashKey);
    const duration = Date.now() - startTime;
    const deletedCount = db.prepare('SELECT changes()').get()['changes()'];

    if (deletedCount > 0) {
      console.log(`[SQLITE CACHE] Deleted cached hash ${hashKey} for ${serviceKey} in ${duration}ms`);
    }
    return { success: true, deletedCount };
  } catch (error) {
    console.error(`[SQLITE CACHE] Error deleting cached hash: ${error.message}`);
    return { success: false, message: error.message };
  }
}

export async function getCachedRecord(service, hash, options = {}) {
  if (!isEnabled()) {
  ...
  try {
    ...
    const releaseKeyFilter = options?.releaseKey ? ' AND releaseKey = ?' : '';
    const stmt = db.prepare(`
      SELECT fileName, size, data, releaseKey, category, resolution, updatedAt, expiresAt
      FROM cache
      WHERE service = ? AND hash = ?${releaseKeyFilter} AND (expiresAt IS NULL OR expiresAt > CURRENT_TIMESTAMP)
    `);
    ...
    const startTime = Date.now();
    const params = options?.releaseKey
      ? [serviceKey, hashKey, String(options.releaseKey)]
      : [serviceKey, hashKey];
...
export async function clearServiceCache(service) {
  ...
  try {
    ...
    db.prepare('DELETE FROM cache WHERE service = ?').run(serviceKey);
    const torrentCount = db.prepare('SELECT changes()').get()['changes()'];

    // Clear search cache entries (service = 'search' AND hash LIKE '<provider>-%')
    db.prepare('DELETE FROM cache WHERE service = ? AND hash LIKE ?').run('search', `${serviceKey}-%`);
    const searchCount = db.prepare('SELECT changes()').get()['changes()'];

    const duration = Date.now() - startTime;
    const totalCount = torrentCount + searchCount;

    console.log(`[SQLITE CACHE] Cleared ${totalCount} cache entries for ${serviceKey} in ${duration}ms (${torrentCount} torrent, ${searchCount} search)`);
    return { success: true, deletedCount: totalCount, torrentCount, searchCount };
  } catch (error) {
    console.error(`[SQLITE CACHE] Error clearing service cache: ${error.message}`);
    return { success: false, message: error.message };
  }
}
 ...
 * This is used to check against cached hashes in parallel with live scraper execution.
 ...
 * @returns {Promise<Array>} Array of cached torrent objects with hash, fileName, size, etc.
 */
export async function getCachedScraperResults(type, imdbId, season = null, episode = null) {
  ...
  try {
    ...
    // Build release key pattern
    const releaseKey = `${type}:${imdbId}`;

    if (debug) {
      console.log(`[SQLITE CACHE] Getting cached scraper results for ${releaseKey}`);
    }

    const startTime = Date.now();

    // Query for both 'scraper-results' service and any debrid service results
    const stmt = db.prepare(`
      SELECT hash, fileName, size, data, releaseKey, category, resolution, updatedAt
      FROM cache
      WHERE (service = 'scraper-results' OR service LIKE '%debrid%' OR service = 'realdebrid' OR service = 'alldebrid' OR service = 'torbox' OR service = 'premiumize')
        AND releaseKey LIKE ?
        AND (expiresAt IS NULL OR expiresAt > CURRENT_TIMESTAMP)
    ...
    const results = stmt.all(`${releaseKey}%`);
    const duration = Date.now() - startTime;
    ...
    console.log(`[SQLITE CACHE] Found ${parsed.length} cached scraper results for ${releaseKey} in ${duration}ms`);
    return parsed;
  } catch (error) {
    console.warn(`[SQLITE CACHE] Error getting cached scraper results: ${error.message}`);
    return [];
  }
}
 ...
 * Used for quick hash lookup without full torrent data.
 * @param {string} type - Content type ('movie' or 'series')
 * @param {string} imdbId - IMDB ID
...
export async function getCachedHashesForRelease(type, imdbId) {
  ...
  try {
    ...
    const hashes = new Set(results.map(row => row.hash.toLowerCase()));
    console.log(`[SQLITE CACHE] Found ${hashes.size} cached hashes for ${releaseKey} in ${duration}ms`);
    return hashes;
  } catch (error) {
    console.warn(`[SQLITE CACHE] Error getting cached hashes for release: ${error.message}`);
    return new Set();
  }
}
...
export async function cleanupHttpStreamsCache(days = null) {
  if (!isEnabled()) {
    console.log('[SQLITE CACHE] SQLite cache is not enabled, skipping HTTP streams cleanup');
  ...
  try {
    ...
    console.log(`[SQLITE CACHE] Cleaned up ${deletedCount} HTTP streams cache entries older than ${ttlDays} days in ${duration}ms`);
    return { success: true, deletedCount };
  } catch (error) {
    console.error(`[SQLITE CACHE] Error cleaning up HTTP streams cache: ${error.message}`);
    return { success: false, deletedCount: 0 };
  }
}
