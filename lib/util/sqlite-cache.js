...
function setupCleanupJob() {
  ...
  // Run startup cleanup for HTTP streams
  runHttpStreamsCleanup();
  ...
  cleanupIntervalId = setInterval(() => {
    try {
      ...
      // Also clean up old HTTP streams entries based on updatedAt
      runHttpStreamsCleanup();
    } catch (error) {
...
// Internal helper to run HTTP streams cleanup synchronously
function runHttpStreamsCleanup() {
  if (!db) return;

  const ttlDays = Number(config.HTTP_STREAMS_CACHE_TTL_DAYS || 30);
  ...
  try {
    ...
    if (deletedCount > 0) {
      console.log(`[SQLITE CACHE] HTTP streams cleanup: removed ${deletedCount} entries older than ${ttlDays} days in ${duration}ms`);
    } else if (debug) {
      console.log(`[SQLITE CACHE] HTTP streams cleanup: no old entries to remove (checked in ${duration}ms)`);
    }
  } catch (error) {
    console.error(`[SQLITE CACHE] Error in HTTP streams cleanup: ${error.message}`);
  }
}
