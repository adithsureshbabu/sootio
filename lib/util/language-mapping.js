...
export function detectLanguagesFromTitle(title) {
  ...
  const blacklistPatterns = [
    ...
    /\bno[\s._-]lang\b/i,         // "No Lang" (e.g., "No Lang Movie")
  ];
  ...
  // Detect standalone 'multi' and 'dual' keywords (common in torrent names)
  // Do this before the main loop to handle MULTi without requiring 'audio'
  if (/\bmulti\b/i.test(titleLower)) {
    // Check if it's 'multi audio' or 'multi subs' first
    if (/\bmulti[\s.\-_]*(audio|subs?)\b/i.test(titleLower)) {
      // Will be detected as 'multi audio' or 'multi subs' in the loop
    } else {
      // Standalone MULTI
      detected.push('multi');
    }
  }
  if (/\bdual\b/i.test(titleLower)) {
    // Check if it's full 'dual audio' phrase
    if (/\bdual[\s.\-_]*audio\b/i.test(titleLower)) {
      // Will be detected as 'dual audio' in the loop
    } else {
      // Standalone DUAL
      detected.push('dual');
    }
...
export function filterStreamsByLanguage(streams, selectedLanguages) {
  ...
  // Check if English is in the selection (for fallback behavior)
  const includesEnglish = selected.includes('english');

  const beforeCount = streams.length;

  const filtered = streams.filter(stream => {
    // Detect languages from title and name
    const text = (stream.title || '') + ' ' + (stream.name || '');
    const detectedLanguages = detectLanguagesFromTitle(text);
    ...
    // - If English is selected, keep it (most unlabeled content is English)
    // - Otherwise, keep it only if englishOnly mode (for backwards compatibility)
    if (combinedLanguages.length === 0) {
      return includesEnglish || englishOnly;
    }
  ...
  if (filtered.length !== beforeCount) {
    console.log(`[LANGUAGE-FILTER] Filtered ${beforeCount - filtered.length} streams by language (${selected.join(', ')}), kept ${filtered.length}`);
  }
