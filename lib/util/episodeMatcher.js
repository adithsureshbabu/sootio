...
const STOPWORDS = new Set([
  ...
  // release/source/codec/language tokens
  'bd','br','web','dl','webrip','bdrip','brrip','remux','dual','audio','dub','sub','multi',
  'hevc','avc','x264','x265','h264','h265','aac','opus','flac','ac3','dts','hdr','sdr',
  'vostfr','vf','vo','dubbed','subbed','bluray','cr','funi','amzn','nf',
  'uncensored','repack','proper','extended','directors','cut'
]);
...
function hasSeasonOnlyMarker(str, season) {
  ...
  const s = sNum.padStart(2, '0');
  const rx1 = new RegExp('(?:^|[^a-z0-9])s\\s*(?:0*' + sNum + ')(?!\\s*[eE]\\s*\\d)', 'i');
  const rx2 = new RegExp('(?:^|[^a-z0-9])season\\s*' + sNum + '(?:\\b|\\D)', 'i');
...
export function matchesCandidateTitle(candidate, ctx, opts = {}) {
  ...
  for (const raw of values) {
    ...
    if (nonStopwordExtras.length > 8) continue; // Allow up to 8 extra tokens
