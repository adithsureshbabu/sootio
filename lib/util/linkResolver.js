      ...
      if (keyMatch && resumeUrl.includes('/zfile/') && post) {
        ...
        try {
          ...
          if (postData && (postData.url || postData.visit_url)) {
            ...
            const pollAttemptsRaw = parseInt(process.env.DRIVESEED_TOKEN_POLL_ATTEMPTS || process.env.UHDMOVIES_ZFILE_POLL_ATTEMPTS || '2', 10);
            const pollDelayRaw = parseInt(process.env.DRIVESEED_TOKEN_POLL_DELAY_MS || process.env.UHDMOVIES_ZFILE_POLL_DELAY_MS || '1500', 10);
