...
async function defaultTryInstantDownload($, { post, origin, log = console }) {
  ...
  try {
    ...
    if (urlParam) {
      ...
      if (urlParam.includes('workers.dev') || urlParam.includes('video-leech.pro')) {
  ...
  try {
    ...
    if (keys) {
      ...
      log.log('[LinkResolver] defaultTryInstantDownload: googleusercontent direct link found - skipping');
      return null;
    } else if (instantLink.includes('workers.dev') || instantLink.includes('video-leech.pro') || instantLink.includes('cdn.video-leech.pro')) {
