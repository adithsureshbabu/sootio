      ...
      try {
        ...
        if (finalUrl) {
          if (finalUrl.includes('googleusercontent.com')) {
            log.log('[LinkResolver] defaultTryInstantDownload: redirect landed on googleusercontent - will try validating original link');
            if (validate) {
              try {
                const ok = await validate(instantLink);
                if (ok) {
                  log.log('[LinkResolver] defaultTryInstantDownload: original video-leech link validated');
                  return instantLink;
                }
              } catch (e) {
                log.log(`[LinkResolver] defaultTryInstantDownload: validation error on original link: ${e.message}`);
              }
            }
          } else {
