...
async function defaultTryInstantDownload($, { post, get, origin, log = console }) {
      ...
      try {
        ...
        if (finalUrl) {
          if (finalUrl.includes('googleusercontent.com')) {
            log.log('[LinkResolver] defaultTryInstantDownload: redirect landed on googleusercontent - skipping');
            return null;
          }
          if (finalUrl.includes('workers.dev') || finalUrl.includes('driveleech') || finalUrl.includes('driveseed')) {
        ...
        if (resp?.data) {
          ...
          const workersMatch = html.match(/https?:\/\/[^"']*workers\.dev[^"'\\s]+/i);
          if (workersMatch && !workersMatch[0].includes('googleusercontent.com')) {
            log.log('[LinkResolver] defaultTryInstantDownload: extracted workers.dev link from HTML');
            return workersMatch[0];
          }
        }
      } catch (err) {
...
async function defaultTryResumeCloud($, { origin, get, post, validate, log = console }) {
      ...
      if (keyMatch && resumeUrl.includes('/zfile/') && post) {
        ...
        try {
          ...
          if (postData && (postData.url || postData.visit_url)) {
            const tokenUrl = postData.visit_url || postData.url;
            ...
            // If no workers.dev link yet, the file may still be processing
            // For now, we can't wait indefinitely, so log and continue
            log.log('[LinkResolver] DriveSeed zfile: no workers.dev link found on token page (file may still be processing)');
          }
        } catch (postError) {
...
async function extractFinalDownloadFromFilePage($, {
  ...
  // Try known methods
  const methods = [
    async () => await tryResumeCloud($, { origin, get, post, validate, log }),
