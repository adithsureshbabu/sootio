// This file has been removed as part of the refactoring to use only the shared cache
// All caching is now handled through the cache backend in cache-store.js

export function get(scraperName, query, config) {
    console.log(`[SCRAPER CACHE] In-memory cache removed, using shared cache only for ${scraperName}:${query}`);
    return null;
}
