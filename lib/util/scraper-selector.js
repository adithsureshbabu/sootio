...
export async function orchestrateScrapers({
  ...
  // Use SCRAPER_RETURN_TIMEOUT_MS if set, otherwise use 80% of SEARCH_TIMEOUT_MS
  const searchTimeout = parseInt(process.env.SEARCH_TIMEOUT_MS || '30000', 10);
  const returnTimeout = SCRAPER_RETURN_TIMEOUT_MS || Math.floor(searchTimeout * 0.8);
  ...
  // Continue remaining scrapers in background if we returned early
  const remainingScrapers = selectedTasks.filter(t => !completedScrapers.has(t.name));
  if (remainingScrapers.length > 0) {
    console.log(`[${logPrefix}] ${remainingScrapers.length} scrapers still running in background: ${remainingScrapers.map(t => t.name).join(', ')}`);

    // Background task to collect remaining results and save to cache
    setImmediate(async () => {
      try {
        // Wait for all remaining scrapers to complete
        await Promise.allSettled(scraperPromises);

        const backgroundResults = collectedResults.filter(r => !foregroundResults.includes(r));
        const totalResults = collectedResults.length;

        console.log(`[${logPrefix}] Background scrapers completed: ${backgroundResults.length} additional results (total: ${totalResults})`);

        // Save all collected hashes to cache for future requests
        if (SqliteCache.isEnabled() && collectedResults.length > 0) {
          try {
            ...
            if (hashesToCache.length > 0) {
              ...
              console.log(`[${logPrefix}] Cached ${hashesToCache.length} hashes from background scrapers`);
            }
          } catch (cacheError) {
            console.error(`[${logPrefix}] Failed to cache background scraper results:`, cacheError.message);
          }
        }
      } catch (bgError) {
        console.error(`[${logPrefix}] Background scraper continuation error:`, bgError.message);
      }
    });
  }
