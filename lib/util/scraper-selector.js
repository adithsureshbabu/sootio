...
function classifyScraperError(error) {
  ...
  // Treat abort signals (from search coordinator early return) as 'aborted', not failures
  if (error.name === 'AbortError' || error.code === 'ERR_ABORTED' || message === 'scraper aborted' || message === 'aborted') return 'aborted';
  // Hard timeout is a coordination cap (stop blocking the user), not a sign the scraper is broken
  if (message.includes('scraper hard timeout')) return 'aborted';
...
export function getEnabledScrapers(userConfig = {}, logPrefix = 'SCRAPER', forceAll = false) {
  ...
  const scraperMap = {
    ...
    'limetorrents': config.LIMETORRENTS_ENABLED,
...
export async function orchestrateScrapers({
  ...
  const addScraperTasks = (cfg, key) => {
    ...
    if (shouldEnableScraper('limetorrents', enabledScrapers)) addScraperTask('limetorrents', () => scrapers.searchLimeTorrents(key, scraperSignal, cfg, logPrefix));
  ...
  const scraperPromises = selectedTasks.map((task, index) => {
    ...
    const wrappedPromise = (async () => {
      ...
      try {
        ...
        if (SCRAPER_PERF_ENABLED) {
          // Always record as success — 0-result runs are valid (no matches for this query).
          // The hard timeout already fires at 10s (before the scraper's own 15s timeout), so
          // we can't distinguish "scraper timed out internally" from "no results" via duration here.
