...
class SearchCoordinator {
  constructor() {
    ...
    const configuredTimeout = parseInt(process.env.SEARCH_TIMEOUT_MS || '30000', 10);
    // Clamp to a sane floor (20s) to avoid premature aborts under load
    this.searchTimeout = Math.max(20000, Number.isFinite(configuredTimeout) ? configuredTimeout : 30000);
