...
class AdRateLimiter {
  constructor({ ratePerMinute = 600, ratePerSecond = 12, concurrency = 50, maxRetries = 5, maxQueueSize = 500 } = {}) {
    ...
    this.queue = [];
    this.running = 0;
    this.concurrency = Math.min(concurrency, 30); // Increased from 10 - allows higher throughput while still protecting the API
    ...
    // Minute tokens: refill 10 tokens every second (600/min = 10/sec)
    const minuteBatchSize = 10;
    const minuteRefillRate = 1000; // 1 second between batches
    this.minuteRefillInterval = setInterval(() => {
      if (this.minuteTokens < this.minuteCapacity) {
        this.minuteTokens = Math.min(this.minuteCapacity, this.minuteTokens + minuteBatchSize);
        this._drain();
      }
    }, minuteRefillRate);

    // Second tokens: refill all every second (12/sec)
    this.secondRefillInterval = setInterval(() => {
      this.secondTokens = this.secondCapacity; // Reset to full every second
      this._drain();
    }, 1000);
  }
