...
async function getMeta(type, imdbId) {
    try {
        ...
        if (meta && process.env.ENABLE_IMDB_ALTERNATIVE_TITLES !== 'false') {
            // Run IMDB fetch in background - don't wait for it
            fetchImdbAlternativeTitles(imdbId).then(altTitles => {
                if (altTitles.length > 0) {
                    meta.alternativeTitles = altTitles;
                    // Update cache with alternative titles
                    metaCache.set(cacheKey, { data: meta, timestamp: Date.now() });
                    console.log(`[Cinemeta] Background: added ${altTitles.length} alternative titles to cache for ${imdbId}`);
                }
            }).catch(err => {
                console.log(`[Cinemeta] Background: failed to fetch alternative titles for ${imdbId}`);
            });
        }
