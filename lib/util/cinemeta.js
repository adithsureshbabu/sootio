...
const CINEMETA_CACHE_TTL_MS = parseInt(process.env.CINEMETA_CACHE_TTL_MS || '300000', 10); // 5 minutes
...
const CINEMETA_BASE_URLS = (process.env.CINEMETA_BASE_URLS || 'https://v3-cinemeta.strem.io,https://cinemeta.strem.io,https://cinemeta.strem.fun')
    .split(',')
...
async function getMeta(type, imdbId) {
    ...
    if (shouldCheckDb) {
        const dbCached = await getCachedMetaFromDb(type, imdbId);
        if (dbCached.hit) {
    ...
    if (cached && cacheAgeMs != null) {
        ...
        if (!metaFetchInFlight.has(cacheKey)) {
            const refreshPromise = (async () => {
                return await fetchMeta(type, imdbId, cacheKey);
            })();
    ...
    const fetchPromise = fetchMeta(type, imdbId, cacheKey);
...
async function fetchMeta(type, imdbId, cacheKey) {
    ...
    const shouldFallbackToImdb = CINEMETA_FALLBACK_TO_IMDB && (
        (lastError && isRetryableCinemetaError(lastError)) || (lastStatus && lastStatus >= 500)
    );
    ...
    if (lastError && isRetryableCinemetaError(lastError)) {
        const cached = metaCache.get(cacheKey);
        if (cached?.data) {
