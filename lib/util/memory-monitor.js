...
class MemoryMonitor {
  constructor(options = {}) {
    ...
    this.isEnabled = true;
    this.thresholdCooldownMs = options.thresholdCooldownMs || 60000; // 60 seconds
    this.lastThresholdHandledAt = 0;
  ...
  logMemoryUsage(level = 'info', message = 'Memory usage check') {
    ...
    if (isRssExceeded || isHeapExceeded) {
      ...
      this.handleThresholdExceeded(memory);
    } else if (level === 'debug') {
  ...
  setThresholdHandler(handler) {
    this.onThresholdExceeded = typeof handler === 'function' ? handler : null;
  }

  handleThresholdExceeded(memory) {
    if (!this.onThresholdExceeded) return;
    const now = Date.now();
    if (now - this.lastThresholdHandledAt < this.thresholdCooldownMs) {
      return;
    }
    this.lastThresholdHandledAt = now;
    try {
      this.onThresholdExceeded(memory);
    } catch (error) {
      logger.error(`[MEMORY] Threshold handler error: ${error.message}`);
    }
  }
}
...
const memoryMonitor = new MemoryMonitor({
  ...
  thresholdCooldownMs: parseInt(process.env.MEMORY_THRESHOLD_COOLDOWN_MS || '60000') // 60s by default
});
