...
const SID_CACHE_TTL = parseInt(process.env.UHDMOVIES_SID_CACHE_TTL) || (5 * 60 * 1000); // 5 minutes
...
export async function resolveSidToDriveleech(sidUrl) {
  ...
  const resolverPromise = (async () => {
    ...
    const MAX_RETRIES = parseInt(process.env.UHDMOVIES_SID_MAX_RETRIES) || 1; // keep retries low for speed (attempts = MAX_RETRIES + 1)
    const RETRY_DELAY = parseInt(process.env.UHDMOVIES_SID_RETRY_DELAY) || 800; // faster retry cadence
    const REQUEST_TIMEOUT = parseInt(process.env.UHDMOVIES_SID_TIMEOUT) || 12000; // lower timeout to fail fast
