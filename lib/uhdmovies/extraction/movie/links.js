...
export async function extractDownloadLinks(moviePageUrl, targetYear = null) {
  try {
    ...
    // ONLY select links with the maxbutton class (proper Download buttons), not bare text links
    $('a[href*="tech.unblockedgames.world"].maxbutton, a[href*="tech.examzculture.in"].maxbutton').each((index, element) => {
