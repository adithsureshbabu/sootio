...
export function formatStreamsWithFlags(cachedLinks) {
  ...
  const streams = cachedLinks.map((streamInfo) => {
    try {
      ...
      // Skip any entries that did not confirm Resume Cloud (guards against stale cache)
      if (streamInfo.resumeCloud === false || streamInfo.__invalid) {
        console.log(`[UHDMovies] Skipping stream without Resume Cloud confirmation: ${streamInfo.rawQuality || streamInfo.quality}`);
        return null;
      }
