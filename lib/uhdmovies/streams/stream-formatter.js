...
export function formatStreamsWithFlags(cachedLinks) {
  ...
  const streams = cachedLinks.map((streamInfo) => {
    try {
      // Check if streamInfo is null or undefined
      if (!streamInfo) {
        console.log(`[UHDMovies] Null or undefined streamInfo, skipping`);
        return null;
      }
      ...
      // Skip any entries marked as invalid during extraction
      // In lazy resolution mode, be more permissive and let the resolver handle validation
      if (streamInfo.__invalid) {
        console.log(`[UHDMovies] Skipping stream marked as invalid: ${streamInfo.rawQuality || streamInfo.quality}`);
        return null;
      }
      ...
      const filename = cleanedFilename || rawQuality;
      const behaviorHints = { bingeGroup: 'sootio-uhdmovies' };
      if (filename) {
  ...
  // Filter out streams that are definitely not video files
  // For preview mode (lazy resolution), be more permissive and allow potentially unknown quality streams
  // since they will be validated on click
  const validStreams = streams.filter(stream => {
    // Check if the stream is an archive file (zip, rar, 7z, pack) - these should always be filtered out
    ...
    // For preview/lazy-resolution streams, allow unknown quality since validation will happen on click
    if (stream.needsResolution) {
      // Only filter out if it's a known non-video format or archive
      return true;
    }

    // For streams that were pre-validated (not lazy), apply strict quality filtering
    const isUnknown = stream.quality === 'Unknown Quality' ||
                      stream.fullTitle === 'Unknown Quality' ||
                      stream.size === 'Unknown';
    if (isUnknown) {
      console.log(`[UHDMovies] Filtering out unknown quality stream (not lazy resolution)`);
      return false;
    }
