...
// Extract helper: grab value after a label inside list-group items on DriveSeed pages
function extractLabelValue($, labels = [], rawHtml = '') {
  for (const label of labels) {
    ...
    // Fallback: search any element containing the label text
    const any = $('*').filter((i, el) => $(el).text().toLowerCase().includes(label.toLowerCase())).first();
    if (any.length > 0) {
      const txt = any.text().trim();
      const m2 = txt.match(new RegExp(`${label}\\s*:?\\s*([^\\n]+)`, 'i'));
      if (m2 && m2[1]) return m2[1].trim();
    }

    // Fallback: regex over raw HTML as last resort
    if (rawHtml) {
      const m3 = rawHtml.match(new RegExp(`${label}\\s*:?[^<\\n]*>([^<\\n]+)<`, 'i'));
...
// Normalize a size string (strip /E markers and keep first size/range)
function normalizeSize(text) {
  if (!text) return null;
  const sizeRegex = /([0-9.,]+\s*[KMGT]B(?!ps)(?:\s*-\s*[0-9.,]+\s*[KMGT]B(?!ps)?)*)(?:\s*\/\s*E.*)?/i;
  const m = text.match(sizeRegex);
...
export async function extractAndValidateLinks(matchingResult, matchingResults, scoredResults, downloadInfo, mediaType, season, episode, year, extractTvShowDownloadLinks, extractDownloadLinks) {
  ...
  if (candidateLinks.length > 0) {
      ...
      try {
        ...
        try {
          ...
          const hasZfileLinks = $test('a[href*="/zfile/"]').length > 0 ||
            ...
            // Require explicit “Resume Cloud” style button per user request
            const hasResumeCloud = $test('a:contains("Resume Cloud"), a:contains("Cloud Resume Download"), a[href*="/zfile/"]').length > 0 ||
                                   driveleechValidation.data.includes('/zfile/') ||
                                   $test('a[href*="workers.dev"]').filter((_, el) => {
            ...
            if (!hasFileElements) {
              ...
              return null;
            } else if (!hasResumeCloud) {
              console.log(`[UHDMovies] ❌ Missing "Resume Cloud" button on file page, skipping: ${driveleechUrl.substring(0, 100)}...`);
              return null;
            } else {
            ...
            const rawHtml = driveleechValidation.data || '';
            const extractedSize = normalizeSize(extractLabelValue($test, ['size'], rawHtml));
            const extractedName = extractLabelValue($test, ['file name', 'filename', 'name'], rawHtml);
            if (extractedName) {
              console.log(`[UHDMovies] 📄 File name: ${extractedName}`);
            }
            if (extractedSize) {
              console.log(`[UHDMovies] 📦 File size: ${extractedSize}`);
            }

            // Override link info with authoritative values from page when available
            if (extractedSize) {
              linkInfo.size = extractedSize;
            }
            if (extractedName) {
              linkInfo.fileName = extractedName;
            }
        ...
        return {
          ...
          fileName: linkInfo.fileName, // Optionally include file name from DriveSeed page
    ...
    cachedLinks = results.filter(link => link && link.url); // Filter for valid objects with URLs
