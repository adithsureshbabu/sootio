...
export async function getUHDMoviesStreams(imdbId, tmdbId, mediaType = 'movie', season = null, episode = null, config = {}) {
  ...
  try {
    ...
    if (!isLazyLoadEnabled() && cachedLinks && cachedLinks.length > 0) {
      ...
      const resolvedLinks = [];
      // Filter out any null or undefined links before processing
      const validLinks = cachedLinks.filter(link => link && (link.url || link.link));
      console.log(`[UHDMovies] Eager resolution: Processing ${validLinks.length}/${cachedLinks.length} valid links`);
      ...
      // Filter out any null or undefined links before processing, then format as streams
      const validLinks = cachedLinks.filter(link => link && (link.url || link.link));
      console.log(`[UHDMovies] After filtering null/undefined links: ${validLinks.length}/${cachedLinks.length} remain`);

      // Format cached links as streams with encoded URLs for lazy resolution
      validStreams = formatStreamsWithFlags(validLinks.map(link => ({
        ...link,
