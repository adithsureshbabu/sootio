...
async function enrichLinkWithDriveSeedMetadata(linkInfo, timeoutMs = 12000) {
  ...
  try {
    ...
    // In preview mode, be more permissive - if we don't find Resume Cloud, still allow the link
    // The actual resolver will validate if the link works when user clicks it
    ...
    // Don't mark as invalid in preview mode - allow all links to pass through for on-demand validation
    if (!hasResumeCloud) {
      console.log(`[UHDMovies] (preview metadata) Missing Resume Cloud on DriveSeed page, allowing link to pass through for on-demand validation: ${linkInfo.rawQuality || linkInfo.quality}`);
    }

    linkInfo.resumeCloud = hasResumeCloud;
    ...
    linkInfo.resumeCloud = false; // Mark as false on error but don't invalidate
  }
...
export async function extractLinksWithoutValidation(
  ...
  if (previewLinks.length === 0 && candidateLinks.length > 0) {
    ...
    // Filter out null/undefined values from candidateLinks before mapping
    const validCandidateLinks = candidateLinks.filter(link => link && link.link);
    previewLinks = validCandidateLinks.map(linkInfo => ({
      quality: linkInfo.quality,
      rawQuality: linkInfo.rawQuality,
      url: linkInfo.link,
      size: linkInfo.size || 'Unknown',
