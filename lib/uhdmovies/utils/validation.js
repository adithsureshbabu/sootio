...
const VALIDATION_TIMEOUT = parseInt(process.env.UHDMOVIES_VALIDATION_TIMEOUT) || 2500; // Configurable timeout, default 2.5 seconds to match HTTP streams
...
export async function validateVideoUrl(url, timeout = VALIDATION_TIMEOUT) {
  ...
  // Hard block googleusercontent links (user request: never use these)
  try {
    const hostname = new URL(url).hostname;
    if (hostname.includes('googleusercontent.com')) {
      console.log('[UHDMovies] Filtering out googleusercontent.com link (skipping validation entirely)');
      return false;
    }
  } catch {
    // If URL parsing fails, proceed with normal validation
  }
