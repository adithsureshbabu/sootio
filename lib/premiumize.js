...
async function search(apiKey, type, id, userConfig = {}) {
    ...
    const { signal, cleanup: cleanupAbort } = debridHelpers.createAbortControllerWithTimeout(SEARCH_TIMEOUT_MS, 'PM');
    ...
    try {
        ...
        // OPTIMIZED: Check SQLite cache FIRST to avoid redundant API calls
        ...
        try {
            if (sqliteCache?.isEnabled()) {
                ...
                console.log(`[${LOG_PREFIX}] SQLite returned ${cachedHashesFromSqlite.size}/${hashes.length} cached hashes`);
            }
        } catch (error) {
            console.error(`[${LOG_PREFIX} MONGO] Error getting cached hashes: ${error.message}`);
        }

        // OPTIMIZED: ONLY call Premiumize API for hashes not in SQLite (saves API bandwidth/rate limits)
        const hashesToCheck = hashes.filter(h => !cachedHashesFromSqlite.has(h.toLowerCase()));
        let cachedResults = [];

        if (hashesToCheck.length > 0) {
            console.log(`[${LOG_PREFIX}] SQLite missing ${hashesToCheck.length} hashes, checking Premiumize API...`);
            cachedResults = await checkCache(apiKey, hashesToCheck);
            console.log(`[${LOG_PREFIX}] Premiumize API returned ${cachedResults.length} cached results`);
        } else {
            console.log(`[${LOG_PREFIX}] ⚡ SKIPPED Premiumize API check (SQLite has all ${cachedHashesFromSqlite.size} hashes!)`);
        }

        // Combine: assume SQLite-cached hashes are still cached (they were checked before)
        const apiCachedHashes = new Set(cachedResults.map(c => c.infoHash.toLowerCase()));
        const cachedHashes = new Set([...cachedHashesFromSqlite, ...apiCachedHashes]);
        const torrentsToProcess = torrents.filter(t => cachedHashes.has(t.InfoHash.toLowerCase()));
        ...
        const processedTorrentsPromises = torrentsToProcess.map(async (torrent) => {
            ...
            if (type === 'series') {
                ...
                if (directDownload.content && Array.isArray(directDownload.content) && directDownload.content.length > 0) {
                    ...
                    if (episodeFile) {
                        const definitiveTitle = `${cacheInfo?.name || torrent.Title || torrent.name}/${episodeFile.path}`;
                        return {
                ...
                if (directDownload.location && isVideo(directDownload.filename)) {
                    ...
                    if (pttInfo.season === s && pttInfo.episode === e) {
                        const definitiveTitle = cacheInfo?.name || torrent.Title || torrent.name;
                        return {
                            name: definitiveTitle,
                            info: PTT.parse(definitiveTitle),
                            size: cacheInfo?.size || torrent.Size || torrent.size || 0,
                            seeders: torrent.Seeders || torrent.seeders || 0,
                            url: magnet,
                            source: 'premiumize',
                            hash: (torrent.InfoHash || '').toLowerCase(),
