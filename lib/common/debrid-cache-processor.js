...
import { getResolutionFromName, formatSize, getCodec, resolutionOrder, normalizeInfoHash } from './torrent-utils.js';
...
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null, satisfiedQuotas = {}, bypassQuotas = false) {
    ...
    const allHashes = torrents
        .map(t => normalizeInfoHash(t.InfoHash || t.infoHash || t.hash || ''))
        .filter(Boolean);
    ...
    const enrichedTorrents = torrents.map(raw => {
        ...
        const infoHash = normalizeInfoHash(raw.InfoHash || raw.infoHash || raw.hash || '');
    ...
    const inspectAllSeasonPacks = async () => {
        ...
        for (const pack of uniqueSeasonPacks) {
            if (validSeasonPackHashes.has(pack.InfoHash.toLowerCase())) {
                ...
                const inspectionResult = validSeasonPackHashes.get(pack.InfoHash.toLowerCase());
                let resultToPush;
