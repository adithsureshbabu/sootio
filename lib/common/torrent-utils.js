...
/**
 * Extracts the actual filename from a file path.
 * Only returns if it looks like an actual video file (has video extension).
 * @param {string} path - The file path or name.
 * @returns {string|undefined} - The filename if valid, undefined otherwise.
 */
export function extractFileName(path) {
    ...
    // Only return if it has a video extension
    const extension = filename.toLowerCase().substring(filename.lastIndexOf('.'));
...
export function normalizeInfoHash(value) {
    ...
    if (/^[a-f0-9]{40}$/i.test(candidate) || /^[a-z2-7]{32}$/i.test(candidate)) {
        return candidate;
    }

    const fromMagnet = getHashFromMagnet(raw);
    if (fromMagnet && (/^[a-f0-9]{40}$/i.test(fromMagnet) || /^[a-z2-7]{32}$/i.test(fromMagnet))) {
        return fromMagnet.toLowerCase();
    }
