...
async function checkTorboxCache(apiKey, hashes) {
    ...
    try {
        ...
        const response = await torboxRequest(
            // TorBox expects `hashes` as a JSON array, not a comma-separated string.
            // list_files=true returns file listings per hash, avoiding extra API calls for pack inspection
            () => axios.post(url, { hashes: hexHashes, format: 'object', list_files: true }, debridProxyManager.getAxiosConfig('torbox', { headers })),
            'cache check'
        );
        ...
        if (response.data?.success && typeof response.data.data === 'object') {
            ...
            for (const [hash, entry] of Object.entries(response.data.data)) {
                const h = hash.toLowerCase();
                cachedHashes.add(h);
                // Store file listings if returned by the API
