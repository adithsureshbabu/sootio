...
function isCanceledTorboxError(error) {
    if (!error) return false;
    if (axios.isCancel?.(error)) return true;

    const code = error?.code || error?.cause?.code;
    if (code === 'ERR_CANCELED' || code === 'ERR_ABORTED') return true;
    if (error?.name === 'AbortError') return true;

    const message = String(error?.message || '').toLowerCase();
    return message === 'canceled' || message === 'cancelled' || message.includes('request aborted');
}

function isRetryableTorboxError(error) {
    if (isCanceledTorboxError(error)) return false;
    ...
    if (message === 'aborted' || message.includes('stream has been aborted')) return true;
    if (code === 'ERR_BAD_RESPONSE' && message.includes('aborted')) return true;
    return false;
}
...
async function searchTorboxTorrents(apiKey, type, id, userConfig = {}) {
    ...
    try {
        ...
        // scraperResults may be [[t1...tN], c1, c2] (array-of-arrays mixed with flat cached items)
        // flatMap normalises everything into a single flat array before filtering
        const flatScraperResults = scraperResults.flatMap(item => Array.isArray(item) ? item : [item]);
        let scraperResultsArray = [flatScraperResults];
...
async function combineAndMarkResults(apiKey, personalFiles, externalSources, episodeInfo = null, cinemetaDetails, seriesCtx = null) {
    ...
    // Packs always need file listings from the API (SQLite only stores the hash, not files).
    // If we skip packs here, inspectCachedPacks gets an empty cachedFiles map and returns 0 results.
    const packHashes = new Set(
        franchiseFiltered
            .filter(isPack)
            .map(t => (t.InfoHash || t.infoHash || t.hash || "").toString().toLowerCase())
    );
    const remaining = hashesToCheck.filter(h => !cachedHashes.has(h) || packHashes.has(h));
    if (remaining.length > 0) {
        const remote = await checkTorboxCache(apiKey, remaining);
        remote.cachedHashes.forEach(h => cachedHashes.add(h));
        if (remote.cachedFiles) {
            remote.cachedFiles.forEach((files, h) => cachedFiles.set(h, files));
        }
    }
    ...
    const packResults = await inspectCachedPacks(apiKey, cachedPacks, episodeInfo, cachedFiles);
...
async function inspectCachedPacks(apiKey, packs, episodeInfo, cachedFiles = new Map()) {
    if (!episodeInfo || packs.length === 0) {
        return [];
    }
    const { season, episode } = episodeInfo;
    const results = [];
    console.log(`[${LOG_PREFIX} PACK INSPECT] Starting inspection for ${packs.length} cached packs (${cachedFiles.size} have pre-fetched file listings).`);
    ...
    for (const pack of packs) {
        try {
            ...
            // Use file listings from checkcached?list_files=true when available
            let files = cachedFiles.get(packHash) || null;
            if (files) {
                console.log(`[${LOG_PREFIX} PACK INSPECT] Got ${files.length} files from checkcached (list_files=true)`);
            } else {
                // No file listing available from checkcached — skip this pack
                // rather than adding it to the user's account uninvited
                console.log(`[${LOG_PREFIX} PACK INSPECT] No file listing from checkcached for ${packHash}, skipping`);
                continue;
            }

            if (!files) {
                console.log(`[${LOG_PREFIX} PACK INSPECT] Could not retrieve file list for pack ${packHash}`);
                continue;
            }

            const matchingFiles = files.filter(file => {
                const fileName = file.short_name || file.name;
                if (!isValidVideo(fileName, file.size, undefined, LOG_PREFIX)) return false;
            ...
            if (matchingFiles.length > 0) {
                ...
                const bestFileName = bestFile.short_name || bestFile.name;
                console.log(`[${LOG_PREFIX} PACK INSPECT] Found matching file: ${bestFileName}`);
                results.push({
                    ...pack,
                    Title: bestFileName,
                    Size: bestFile.size,
...
async function checkTorboxCache(apiKey, hashes) {
    const empty = { cachedHashes: new Set(), cachedFiles: new Map() };
    ...
    try {
        ...
        console.log(`[${LOG_PREFIX}] Sending ${hexHashes.length} hex hashes to TorBox API.`);
        // Use GET with format=list&list_files=true -- the POST/object format does not return files
        // even when list_files=true is set. GET/list does return full file listings.
        const hashParam = hexHashes.join(',');
        const response = await torboxRequest(
            () => axios.get(url, debridProxyManager.getAxiosConfig('torbox', {
                headers,
                params: { hash: hashParam, format: 'list', list_files: true }
            })),
            'cache check'
        );
        ...
        if (response.data?.success && Array.isArray(response.data.data)) {
            ...
            for (const entry of response.data.data) {
                ...
                if (Array.isArray(entry.files) && entry.files.length > 0) {
                    cachedFiles.set(h, entry.files);
                }
            }

            console.log(`[${LOG_PREFIX}] TorBox API returned ${cachedHashes.size} cached hashes (${cachedFiles.size} with file listings)`);
            return { cachedHashes, cachedFiles };
        }
        return empty;
    } catch (error) {
...
async function unrestrictUrl(apiKey, itemId, hostUrl, clientIp) {
    ...
    // Reject URLs that belong to other providers
    if (hostUrl.startsWith('realdebrid:') || hostUrl.startsWith('rd:') || hostUrl.startsWith('ad:') || hostUrl.startsWith('pm:')) {
        console.error(`[${LOG_PREFIX} RESOLVER] Rejecting non-TorBox URL: ${hostUrl}`);
        return null;
    }
...
async function listPersonalFiles(apiKey) {
    try {
        ...
        if (isCanceledTorboxError(error)) {
            console.log(`[${LOG_PREFIX}] listPersonalFiles canceled: ${error.message}`);
            return [];
        }
        if (isRetryableTorboxError(error)) {
            console.warn(`[${LOG_PREFIX}] Failed to list personal files after retries: ${error.message}`);
            return [];
        }
...
export const __testables = { checkTorboxCache };
