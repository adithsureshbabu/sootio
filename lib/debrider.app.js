...
async function search(apiKey, type, id, userConfig = {}) {
    ...
    try {
        ...
        // OPTIMIZED: ONLY call Debrider.app API for hashes not in SQLite (saves API bandwidth/rate limits)
        const hashesToCheck = hashes.filter(h => !cachedHashesFromSqlite.has(h.toLowerCase()));
        let cachedTorrents = [];

        if (hashesToCheck.length > 0) {
            console.log(`[${LOG_PREFIX}] SQLite missing ${hashesToCheck.length} hashes, checking Debrider.app API...`);
            ...
            console.log(`[${LOG_PREFIX}] ⚡ SKIPPED Debrider.app API check (SQLite has all ${cachedHashesFromSqlite.size} hashes!)`);
        }
